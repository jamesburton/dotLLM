...
- **Microsoft Agent Framework ready** — native `Microsoft.Extensions.AI.IChatClient` adapter (`DotLLM.Extensions.AI`) — drop dotLLM into MAF agents in-process, no HTTP
...
| [`DotLLM.Extensions.AI`](https://www.nuget.org/packages/DotLLM.Extensions.AI) | `Microsoft.Extensions.AI.IChatClient` adapter — native Microsoft Agent Framework / MEAI backend |
...
- **2026-06** — **Microsoft Agent Framework compatibility** — new `DotLLM.Extensions.AI` package implementing `Microsoft.Extensions.AI.IChatClient` (`DotLLMChatClient`) over the dotLLM engine. Makes dotLLM a native, in-process backend for the Microsoft Agent Framework (`chatClient.CreateAIAgent(...)`) and the wider `Microsoft.Extensions.AI` ecosystem — streaming, tool calling (`FunctionCallContent`), structured output, and token usage all map onto the engine with no HTTP round-trip. AOT-clean, targets .NET 10, depends only on `Microsoft.Extensions.AI.Abstractions`. 22 unit tests; see `docs/EXTENSIONS_AI.md` ([#327](https://github.com/kkokosa/dotLLM/issues/327))
