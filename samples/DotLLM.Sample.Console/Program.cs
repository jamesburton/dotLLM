...
// Threading: the IModelArchitecture factory does not yet expose ThreadingConfig — the
// Qwen3MoeHybrid direct-dispatch path (which honours --threads N) ships in a follow-up
// PR. All architectures here go through the factory (single-threaded for now); the
// --threads flag is parsed and surfaced for forward-compatibility.
IDisposable LoadModel()
{
    if (threadCount > 1)
    {
        Console.WriteLine($"Threading: --threads {threadCount} requested; ignored on this base (Qwen3MoeHybrid direct-dispatch path lands in a follow-up PR).");
    }
