...
public class VulkanAttentionCoopmatParityTests
{
    ...
    private static void RunParity(int seqQ, int seqKv, int numHeads, int numKvHeads, int headDim, int positionOffset)
    {
        ...
        // attention_f32_sg.spv ships in a follow-up PR (RmsNormF32Kernel
        // ForceSharedReduce gate chain); skip the subgroup variant when the
        // SPV is missing from the build output rather than mis-asserting
        // the dispatch mode.
        bool subgroupSpvExists = System.IO.File.Exists(System.IO.Path.Combine(spvDir, "attention_f32_sg.spv"));
        if (device.HasSubgroupArithmetic && subgroupSpvExists)
        {
    ...
    {
        string? prevShared  = Environment.GetEnvironmentVariable(AttentionF32Kernel.ForceSharedReduceEnvVar);
        string? prevCoopmat = Environment.GetEnvironmentVariable(AttentionF32Kernel.UseCoopmatEnvVar);
        Environment.SetEnvironmentVariable(AttentionF32Kernel.ForceSharedReduceEnvVar,   forceSharedReduce ? "1" : null);
        ...
        {
            Environment.SetEnvironmentVariable(AttentionF32Kernel.ForceSharedReduceEnvVar, prevShared);
