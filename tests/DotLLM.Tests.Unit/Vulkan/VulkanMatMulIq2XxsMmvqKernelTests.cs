...
/// <summary>
/// Tolerance + argmax parity for the dp4a IQ2_XXS MMVQ decode GEMV (issue #339):
/// Q8_1-quantized activation × IQ2_XXS codebook-grid weights (sign·grid → int8 →
/// dp4a), compared to the IQ2_XXS-byte-identical CPU oracle
/// <see cref="Iq2Fixture.CpuGemvIq2Xxs"/>.
/// </summary>
[Trait("Category", "GPU")]
[Collection("VulkanKernels")]
public class VulkanMatMulIq2XxsMmvqKernelTests
{
    ...
    public void Mmvq_MatchesF32Oracle_ArgmaxAndTolerance(int m, int k)
    {
        ...
        using var mmvq = MatMulIq2XxsMmvqKernel.TryCreate(device, spvDir, codebooks)
            ?? throw new Xunit.Sdk.XunitException("matmul_iq2_xxs_mmvq.spv missing or unsupported.");

        var rng = new Random(0x22 + m * 7 + k * 11);
        ...
        Iq2Fixture.AssertFixtureRoundtripIq2Xxs(weightsF32, wq, m, k);
        float[] expected = Iq2Fixture.CpuGemvIq2Xxs(wq, x, m, k);
        ...
        Iq4MmvqParity.Assert(expected, actual, m, k, RelTol, "IQ2_XXS");
    }
}
