...
/// <summary>
/// Tolerance + argmax parity test for the dp4a IQ4_XS MMQ prefill GEMM (issue #343):
/// <c>QuantizeQ8_1RowsKernel</c> (F32 B[N,K] → Q8_1 row-wise) followed by
/// <c>MatMulIq4XsMmqKernel</c> (integer-dot GEMM against IQ4_XS weights — nibbles
/// codebook-decoded to int8 with a per-sub-block 6-bit scale, no min term). Compared
/// against the byte-identical CPU F32 oracle <see cref="Iq4Fixture.CpuGemmIq4Xs"/>.
/// </summary>
[Trait("Category", "GPU")]
[Collection("VulkanKernels")]
public class VulkanMatMulIq4XsMmqKernelTests
{
    ...
    public void Mmq_MatchesF32Oracle_ArgmaxAndTolerance(int n, int m, int k)
    {
        ...
        using var mmq = MatMulIq4XsMmqKernel.TryCreate(device, spvDir)
            ?? throw new Xunit.Sdk.XunitException("matmul_iq4_xs_mmq.spv missing or unsupported.");

        var rng = new Random(0x4D + n * 13 + m * 7 + k * 3);
        float[] weightsF32 = Iq4Fixture.RandomFloats(rng, m * k, range: 0.1f);
        float[] inputB = Iq4Fixture.RandomFloats(rng, n * k, range: 1.0f);

        byte[] weightsIq4Xs = Iq4Fixture.QuantizeRowsIq4Xs(weightsF32, m, k);
        float[] expected = Iq4Fixture.CpuGemmIq4Xs(weightsIq4Xs, inputB, m, k, n);

        long weightsBufBytes = ((long)weightsIq4Xs.Length + 3) & ~3L;
        ...
        device.Upload(new ReadOnlySpan<byte>(weightsIq4Xs), bufW);
