...
/// <summary>
/// End-to-end parity tests for the Vulkan Mamba-3 forward path with Q3_K_M
...
/// full upload-path + RecordMatmul dispatch pipeline for Q3_K is wired through, on
/// top of the kernel-level parity already covered by
/// <see cref="VulkanMatMulQ3KGemvF32KernelTests"/> /
/// <see cref="VulkanMatMulQ3KGemmF32KernelTests"/>.
/// </summary>
/// <remarks>
/// <para>
/// <b>Scope.</b> The three matmul-target projections (<c>in_proj</c>, <c>out_proj</c>,
/// <c>lm_head</c>) run through the Q3_K path. Token embedding always lands as F32
...
/// data is quantised to Q3_K via <see cref="Q3KFixture.QuantizeRows"/> and the raw
/// 110-byte super-block bytes are pinned to the overlay slots
...
/// <c>*QuantTypeOverlay</c> field declares (Q8_0 / Q4_K / Q5_K / Q3_K in Phase 1). The
/// corresponding F32 pointer slots are replaced with freshly-allocated buffers
/// holding dequantised-from-Q3_K values, so the CPU oracle reads the same lossy
/// values the Vulkan path consumes via the Q3_K kernels.
/// </para>
/// <para>
/// <b>Dimensions.</b> All Q3_K contraction axes (<c>HiddenSize</c> for <c>in_proj</c>
/// and <c>lm_head</c>; <c>DInner = NumHeads * HeadDim</c> for <c>out_proj</c>) are
/// bumped to multiples of 256 — a hard requirement of the Vulkan Q3_K matmul kernels.
/// Tolerance abs 5e-3 / rel 1e-3, matching the Q4_K / Q5_K siblings. Q3_K's 6-bit
...
public sealed class VulkanMamba3TransformerModelQ3KForwardTests : IDisposable
{
    // Q3_K-friendly dimensions: HiddenSize must be a multiple of 256 (in_proj / lm_head
    ...
    // Native allocations made by the Q3_K overlay (raw super-block bytes + dequantised-
    // back-to-F32 mirror buffers). Freed in Dispose so memory survives the test methods.
    private readonly List<nint> _q3kAllocs = new();

    public VulkanMamba3TransformerModelQ3KForwardTests()
    {
        _scratch = Path.Combine(Path.GetTempPath(), $"dotllm-m3-q3k-vk-{Guid.NewGuid():N}");
        Directory.CreateDirectory(_scratch);
    }

    public unsafe void Dispose()
    {
        foreach (var p in _q3kAllocs)
    ...
    [SkippableFact]
    public void Forward_Q3K_Prefill_SingleLayer_Siso_MatchesCpuReference()
        => AssertVulkanMatchesCpuSiso(numLayers: 1, seqLen: 4, seed: 7);

    [SkippableFact]
    public void Forward_Q3K_Prefill_MultiLayer_Siso_MatchesCpuReference()
        => AssertVulkanMatchesCpuSiso(numLayers: 2, seqLen: 4, seed: 17);

    [SkippableFact]
    public void Forward_Q3K_Decode_SingleToken_Siso_MatchesCpuReference()
    ...
    private void AssertVulkanMatchesCpuSiso(int numLayers, int seqLen, int seed)
    {
        ...
        string path = Path.Combine(_scratch, $"m3-q3k-L{numLayers}-T{seqLen}-s{seed}.safetensors");
        ...
        float[] cpuLogits;
        {
            ...
            ApplyQ3KOverlayInPlace(weights);
        ...
        for (int c = 0; c < VocabSize; c++)
        {
            ...
            Assert.True(diff <= bar,
                $"Q3_K SISO numLayers={numLayers}, seqLen={seqLen}, col={c}: " +
    ...
    /// <c>in_proj</c>/<c>out_proj</c>) to Q3_K via the test fixture quantiser, attaches
    ...
    /// Q3_K values so the CPU oracle consumes the same effective values as the Vulkan
    /// path. Same approach as the Q4_K / Q5_K siblings.
    /// </summary>
    private unsafe void ApplyQ3KOverlayInPlace(Mamba3Weights weights)
    {
        ...
        // ── lm_head ([vocab, hidden]) ─────────────────────────────────────────
        {
            ...
            nint q3kPtr = QuantizeF32ToQ3K(srcF32, vocab, hidden);
            nint freshF32 = AllocAlignedF32(elems);
            DequantizeQ3KRowsToF32Span(q3kPtr, vocab, hidden,
            ...
            weights.LmHead = new Mamba3TensorHandle(
                ...
                OwnsMemory: false);
            weights.LmHeadQ8Ptr = q3kPtr;
            weights.LmHeadQuantTypeOverlay = QuantizationType.Q3_K;
        }
        ...
        for (int i = 0; i < numLayers; i++)
        {
            ...
            // in_proj [d_in_proj, hidden]  — contraction axis = hidden
            {
                ...
                nint q3kPtr = QuantizeF32ToQ3K(srcF32, rows, cols);
                nint freshF32 = AllocAlignedF32(elems);
                DequantizeQ3KRowsToF32Span(q3kPtr, rows, cols,
                    new Span<float>((void*)freshF32, checked((int)elems)));
                ...
                ov.InProjQ8Ptr = q3kPtr;
                ov.InProjQuantTypeOverlay = QuantizationType.Q3_K;
            }
            ...
            // out_proj [hidden, d_inner]  — contraction axis = d_inner
            {
                ...
                ov.OutProjQ8Ptr = q3kPtr;
                ov.OutProjQuantTypeOverlay = QuantizationType.Q3_K;
            }
    ...
    /// matrix to Q3_K row-stride bytes via <see cref="Q3KFixture.QuantizeRows"/>.
    /// Returns an aligned native allocation that the test owns (freed in Dispose).
    /// <paramref name="cols"/> must be a multiple of the Q3_K super-block size (256).
    /// </summary>
    private unsafe nint QuantizeF32ToQ3K(ReadOnlySpan<float> src, int rows, int cols)
    {
        if ((cols % 256) != 0)
            throw new InvalidOperationException(
                $"Q3_K quantisation requires cols % 256 == 0 (got cols={cols}). Bump fixture dims.");
        // Materialise the source as a managed array — the fixture quantiser takes one.
        float[] arr = new float[(long)rows * cols];
        src.CopyTo(arr);
        byte[] q3kBytes = Q3KFixture.QuantizeRows(arr, rows, cols);

        long totalBytes = q3kBytes.Length;
        nint dst = (nint)NativeMemory.AlignedAlloc((nuint)totalBytes, 64);
        _q3kAllocs.Add(dst);
        new ReadOnlySpan<byte>(q3kBytes).CopyTo(new Span<byte>((void*)dst, checked((int)totalBytes)));
        return dst;
    }

    /// <summary>
    /// Dequantises Q3_K row-stride bytes back into F32 row-major elements at
    /// <paramref name="dst"/>. Used so the CPU oracle consumes the (lossy) dequantised
    /// values matching what the Vulkan side computes via the Q3_K kernels.
    /// </summary>
    private static unsafe void DequantizeQ3KRowsToF32Span(nint q3kPtr, int rows, int cols, Span<float> dst)
    {
        long rowBytes = Dequantize.RowByteSize(cols, QuantizationType.Q3_K);
        for (int row = 0; row < rows; row++)
        {
            nint rowSrc = q3kPtr + (nint)((long)row * rowBytes);
            Dequantize.ToFloat32(rowSrc, cols, QuantizationType.Q3_K, dst.Slice(row * cols, cols));
        }
    }
