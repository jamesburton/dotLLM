...
/// </para>
/// <para>
/// FORWARD-REFERENCE ADAPTATION: the original tests used
/// <c>VulkanDevice.CreateSubmitContext</c> + <c>CopyBufferRangeSynchronous</c>
/// to drive the cmdbuf-based <c>RecordUpdate</c> API. PR #205's scaffold
/// has neither, so this test allocates / submits a one-time command
/// buffer inline. The Vulkan perf chain restores both helpers.
/// </para>
/// </remarks>
[Trait("Category", "GPU")]
...
public class VulkanMlaKvCacheTests
{
    ...
    public void RecordUpdate_AppendsContiguousRows()
    {
        ...
        // Buffers are host-visible under the inline-Allocate adaptation, so
        // Download directly off the cache backing buffers.
        var kNopeOut = new float[4 * kNopeRow];
        var vOut = new float[4 * vRow];
        var kPeOut = new float[4 * kPeRow];
        device.Download(cache.GetKNopeBuffer(0), kNopeOut.AsSpan(0, 4 * kNopeRow));
        device.Download(cache.GetVBuffer(0), vOut.AsSpan(0, 4 * vRow));
        device.Download(cache.GetKPeBuffer(0), kPeOut.AsSpan(0, 4 * kPeRow));
    ...
    public void RecordUpdate_NonContiguousPositions_Throws()
    {
        ...
        // We only need a live command buffer for the RecordUpdate validation to
        // run — RecordUpdate throws before any vkCmd* is issued so we never
        // submit. Just allocate + begin a one-time cmdbuf.
        AllocateAndBeginCommandBuffer(device, out nint cmdBuf);
        try
        {
            Assert.Throws<NotSupportedException>(() =>
                cache.RecordUpdate(cmdBuf, bufKNope, bufV, bufKPe, positions, 2, 0));
        }
        finally
        {
            VulkanApi.vkEndCommandBuffer(cmdBuf);
            VulkanApi.vkFreeCommandBuffers(device.Handle, device.CommandPool, 1, cmdBuf);
        }
    }
    ...
    private static unsafe void UploadAndRecord(
        ...
        int[] positions, int seqLen, int layerIndex)
    {
        ...
        try
        {
            cache.RecordUpdate(cmdBuf, bufKNope, bufV, bufKPe, positions, seqLen, layerIndex);
            ...
            VulkanApi.vkQueueSubmit(device.Queue, 1, submit, 0).ThrowOnError("vkQueueSubmit");
            VulkanApi.vkQueueWaitIdle(device.Queue).ThrowOnError("vkQueueWaitIdle");
        }
        finally
        {
            VulkanApi.vkFreeCommandBuffers(device.Handle, device.CommandPool, 1, cmdBuf);
    ...
    private static void AllocateAndBeginCommandBuffer(VulkanDevice device, out nint cmdBuf)
    {
        ...
        VulkanApi.vkAllocateCommandBuffers(device.Handle, cbai, out cmdBuf)
