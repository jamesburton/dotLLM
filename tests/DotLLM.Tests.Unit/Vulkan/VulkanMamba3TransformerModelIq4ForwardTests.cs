...
/// <summary>
/// End-to-end parity tests for the Vulkan Mamba-3 forward path with IQ4_XS and IQ4_NL
...
/// to IQ4_XS / IQ4_NL. Audit finding H3: the IQ3 GEMV/GEMM kernels are bit-perfect
/// against the CPU oracle (16 Iq4 kernel parity tests in tree), and commit ad6b853
/// added IQ4_XS / IQ4_NL case branches to <see cref="VulkanMamba3TransformerModel"/>'s
...
/// data is quantised to IQ3 via <see cref="Iq4Fixture.QuantizeRowsIq4Xs"/> /
/// <see cref="Iq4Fixture.QuantizeRowsIq4Nl"/> and the raw block bytes are pinned to
...
/// does NOT recognise <see cref="QuantizationType.IQ4_XS"/> or
/// <see cref="QuantizationType.IQ4_NL"/> — only the dense host's
/// <see cref="VulkanWeights"/> has IQ3 keep-on-device predicates. Until that gap is
/// closed, setting <c>LmHeadQuantTypeOverlay = IQ4_XS</c> on a <see cref="Mamba3Weights"/>
...
/// <c>.planning/notes/iq4-per-host-parity-deferred.md</c>.
/// </para>
/// </remarks>
[Trait("Category", "GPU")]
[Collection("VulkanKernels")]
public sealed class VulkanMamba3TransformerModelIq4ForwardTests : IDisposable
{
    ...
    private readonly List<nint> _iq4Allocs = new();

    public VulkanMamba3TransformerModelIq4ForwardTests()
    {
        _scratch = Path.Combine(Path.GetTempPath(), $"dotllm-m3-iq4-vk-{Guid.NewGuid():N}");
        Directory.CreateDirectory(_scratch);
    }

    public unsafe void Dispose()
    {
        foreach (var p in _iq4Allocs)
    ...
    [SkippableFact]
    public void Forward_IQ4_NL_Prefill_SingleLayer_Siso_MatchesCpuReference()
        => AssertVulkanMatchesCpuSiso(QuantizationType.IQ4_NL, numLayers: 1, seqLen: 4, seed: 7);

    [SkippableFact]
    public void Forward_IQ4_NL_Prefill_MultiLayer_Siso_MatchesCpuReference()
        => AssertVulkanMatchesCpuSiso(QuantizationType.IQ4_NL, numLayers: 2, seqLen: 4, seed: 17);

    [SkippableFact]
    public void Forward_IQ4_NL_Decode_SingleToken_Siso_MatchesCpuReference()
        => AssertVulkanMatchesCpuSiso(QuantizationType.IQ4_NL, numLayers: 1, seqLen: 1, seed: 31);

    [SkippableFact]
    public void Forward_IQ4_XS_Prefill_SingleLayer_Siso_MatchesCpuReference()
        => AssertVulkanMatchesCpuSiso(QuantizationType.IQ4_XS, numLayers: 1, seqLen: 4, seed: 47);

    [SkippableFact]
    public void Forward_IQ4_XS_Prefill_MultiLayer_Siso_MatchesCpuReference()
        => AssertVulkanMatchesCpuSiso(QuantizationType.IQ4_XS, numLayers: 2, seqLen: 4, seed: 53);

    [SkippableFact]
    public void Forward_IQ4_XS_Decode_SingleToken_Siso_MatchesCpuReference()
        => AssertVulkanMatchesCpuSiso(QuantizationType.IQ4_XS, numLayers: 1, seqLen: 1, seed: 61);

    private void AssertVulkanMatchesCpuSiso(QuantizationType iq4Type, int numLayers, int seqLen, int seed)
    {
        VulkanMatMulF32KernelTests.SkipIfUnavailable(out string spvDir);

        string path = Path.Combine(_scratch, $"m3-{iq4Type}-L{numLayers}-T{seqLen}-s{seed}.safetensors");
        ...
        float[] vkLogits;
        {
            ...
            // without breaking the CI bar — see .planning/notes/iq4-per-host-parity-deferred.md
            // for the precise next-step (add KeepIq4NlOnDevice / KeepIq4XsOnDevice
            ...
            {
                bool lmGateOpen = vkWeights.LmHeadDeviceQuantType == iq4Type;
                bool inProjGateOpen = true;
                bool outProjGateOpen = true;
                for (int li = 0; li < vkWeights.Layers.Length; li++)
                {
                    if (vkWeights.Layers[li].InProjDeviceQuantType != iq4Type) inProjGateOpen = false;
                    if (vkWeights.Layers[li].OutProjDeviceQuantType != iq4Type) outProjGateOpen = false;
                }
                Skip.IfNot(lmGateOpen && inProjGateOpen && outProjGateOpen,
                    $"VulkanMamba3Weights upload-path predicate does not yet recognise {iq4Type}; " +
                    ...
                    "Add KeepIq4NlOnDevice/KeepIq4XsOnDevice predicates to " +
                    ...
                    ".planning/notes/iq4-per-host-parity-deferred.md for context.");
            }
        ...
        for (int c = 0; c < VocabSize; c++)
        {
            ...
            float vk = vkLogits[c];
            Assert.True(float.IsFinite(cpu), $"non-finite CPU logit {iq4Type} L={numLayers} T={seqLen} col={c}: {cpu}");
            Assert.True(float.IsFinite(vk), $"non-finite Vulkan logit {iq4Type} L={numLayers} T={seqLen} col={c}: {vk}");
            float diff = MathF.Abs(cpu - vk);
            float bar = AbsTol + RelTol * MathF.Abs(cpu);
            Assert.True(diff <= bar,
                $"{iq4Type} SISO numLayers={numLayers}, seqLen={seqLen}, col={c}: " +
        ...
        // same broken output.
        AssertNonDegenerate(vkLogits, iq4Type, numLayers, seqLen);
    }
    ...
    /// <see cref="Iq4Fixture"/>, attaches the raw bytes to the overlay slots on
    ...
    /// siblings.
    /// </summary>
    private unsafe void ApplyIq4OverlayInPlace(Mamba3Weights weights, QuantizationType iq4Type)
    {
        if (iq4Type != QuantizationType.IQ4_NL && iq4Type != QuantizationType.IQ4_XS)
            throw new ArgumentException($"Unexpected IQ3 type {iq4Type}", nameof(iq4Type));
        ...
        // ── lm_head ([vocab, hidden]) ─────────────────────────────────────────
        {
            ...
            nint iq4Ptr = QuantizeF32ToIq4(srcF32, vocab, hidden, iq4Type);
            nint freshF32 = AllocAlignedF32(elems);
            DequantizeIq4RowsToF32Span(iq4Ptr, vocab, hidden, iq4Type,
            ...
            weights.LmHead = new Mamba3TensorHandle(
                ...
                OwnsMemory: false);
            weights.LmHeadQ8Ptr = iq4Ptr;
            weights.LmHeadQuantTypeOverlay = iq4Type;
        }
        ...
        for (int i = 0; i < numLayers; i++)
        {
            ...
            // in_proj [d_in_proj, hidden]  — contraction axis = hidden
            {
                ...
                nint iq4Ptr = QuantizeF32ToIq4(srcF32, rows, cols, iq4Type);
                nint freshF32 = AllocAlignedF32(elems);
                DequantizeIq4RowsToF32Span(iq4Ptr, rows, cols, iq4Type,
                    new Span<float>((void*)freshF32, checked((int)elems)));
                ...
                ov.InProjQ8Ptr = iq4Ptr;
                ov.InProjQuantTypeOverlay = iq4Type;
            }
            ...
            // out_proj [hidden, d_inner]  — contraction axis = d_inner
            {
                ...
                ov.OutProjQ8Ptr = iq4Ptr;
                ov.OutProjQuantTypeOverlay = iq4Type;
            }
    ...
    /// matrix to IQ3 row-stride bytes via <see cref="Iq4Fixture"/>. Returns an aligned
    ...
    /// </summary>
    private unsafe nint QuantizeF32ToIq4(ReadOnlySpan<float> src, int rows, int cols, QuantizationType iq4Type)
    {
        if ((cols % 256) != 0)
        ...
        src.CopyTo(arr);
        byte[] iq4Bytes = iq4Type == QuantizationType.IQ4_NL
            ? Iq4Fixture.QuantizeRowsIq4Nl(arr, rows, cols)
            : Iq4Fixture.QuantizeRowsIq4Xs(arr, rows, cols);

        long totalBytes = iq4Bytes.Length;
        nint dst = (nint)NativeMemory.AlignedAlloc((nuint)totalBytes, 64);
        _iq4Allocs.Add(dst);
        new ReadOnlySpan<byte>(iq4Bytes).CopyTo(new Span<byte>((void*)dst, checked((int)totalBytes)));
        return dst;
    }
    ...
    /// </summary>
    private static unsafe void DequantizeIq4RowsToF32Span(nint iq4Ptr, int rows, int cols,
        QuantizationType iq4Type, Span<float> dst)
    {
        long rowBytes = Dequantize.RowByteSize(cols, iq4Type);
        for (int row = 0; row < rows; row++)
        {
            nint rowSrc = iq4Ptr + (nint)((long)row * rowBytes);
            Dequantize.ToFloat32(rowSrc, cols, iq4Type, dst.Slice(row * cols, cols));
        }
    }
