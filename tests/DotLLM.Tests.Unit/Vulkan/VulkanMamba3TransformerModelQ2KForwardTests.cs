...
/// <summary>
/// End-to-end parity tests for the Vulkan Mamba-3 forward path with Q2_K_M
...
/// full upload-path + RecordMatmul dispatch pipeline for Q2_K is wired through, on
/// top of the kernel-level parity already covered by
/// <see cref="VulkanMatMulQ2KGemvF32KernelTests"/> /
/// <see cref="VulkanMatMulQ2KGemmF32KernelTests"/>.
/// </summary>
/// <remarks>
/// <para>
/// <b>Scope.</b> The three matmul-target projections (<c>in_proj</c>, <c>out_proj</c>,
/// <c>lm_head</c>) run through the Q2_K path. Token embedding always lands as F32
...
/// data is quantised to Q2_K via <see cref="Q2KFixture.QuantizeRows"/> and the raw
/// 84-byte super-block bytes are pinned to the overlay slots
...
/// <c>*QuantTypeOverlay</c> field declares (Q8_0 / Q4_K / Q5_K / Q2_K in Phase 1). The
/// corresponding F32 pointer slots are replaced with freshly-allocated buffers
/// holding dequantised-from-Q2_K values, so the CPU oracle reads the same lossy
/// values the Vulkan path consumes via the Q2_K kernels.
/// </para>
/// <para>
/// <b>Dimensions.</b> All Q2_K contraction axes (<c>HiddenSize</c> for <c>in_proj</c>
/// and <c>lm_head</c>; <c>DInner = NumHeads * HeadDim</c> for <c>out_proj</c>) are
/// bumped to multiples of 256 — a hard requirement of the Vulkan Q2_K matmul kernels.
/// Tolerance abs 5e-3 / rel 1e-3, matching the Q4_K / Q5_K siblings. Q2_K's 6-bit
...
public sealed class VulkanMamba3TransformerModelQ2KForwardTests : IDisposable
{
    // Q2_K-friendly dimensions: HiddenSize must be a multiple of 256 (in_proj / lm_head
    ...
    // Native allocations made by the Q2_K overlay (raw super-block bytes + dequantised-
    // back-to-F32 mirror buffers). Freed in Dispose so memory survives the test methods.
    private readonly List<nint> _q2kAllocs = new();

    public VulkanMamba3TransformerModelQ2KForwardTests()
    {
        _scratch = Path.Combine(Path.GetTempPath(), $"dotllm-m3-q2k-vk-{Guid.NewGuid():N}");
        Directory.CreateDirectory(_scratch);
    }

    public unsafe void Dispose()
    {
        foreach (var p in _q2kAllocs)
    ...
    [SkippableFact]
    public void Forward_Q2K_Prefill_SingleLayer_Siso_MatchesCpuReference()
        => AssertVulkanMatchesCpuSiso(numLayers: 1, seqLen: 4, seed: 7);

    [SkippableFact]
    public void Forward_Q2K_Prefill_MultiLayer_Siso_MatchesCpuReference()
        => AssertVulkanMatchesCpuSiso(numLayers: 2, seqLen: 4, seed: 17);

    [SkippableFact]
    public void Forward_Q2K_Decode_SingleToken_Siso_MatchesCpuReference()
    ...
    private void AssertVulkanMatchesCpuSiso(int numLayers, int seqLen, int seed)
    {
        ...
        string path = Path.Combine(_scratch, $"m3-q2k-L{numLayers}-T{seqLen}-s{seed}.safetensors");
        ...
        float[] cpuLogits;
        {
            ...
            ApplyQ2KOverlayInPlace(weights);
        ...
        for (int c = 0; c < VocabSize; c++)
        {
            ...
            Assert.True(diff <= bar,
                $"Q2_K SISO numLayers={numLayers}, seqLen={seqLen}, col={c}: " +
    ...
    /// <c>in_proj</c>/<c>out_proj</c>) to Q2_K via the test fixture quantiser, attaches
    ...
    /// Q2_K values so the CPU oracle consumes the same effective values as the Vulkan
    /// path. Same approach as the Q4_K / Q5_K siblings.
    /// </summary>
    private unsafe void ApplyQ2KOverlayInPlace(Mamba3Weights weights)
    {
        ...
        // ── lm_head ([vocab, hidden]) ─────────────────────────────────────────
        {
            ...
            nint q2kPtr = QuantizeF32ToQ2K(srcF32, vocab, hidden);
            nint freshF32 = AllocAlignedF32(elems);
            DequantizeQ2KRowsToF32Span(q2kPtr, vocab, hidden,
            ...
            weights.LmHead = new Mamba3TensorHandle(
                ...
                OwnsMemory: false);
            weights.LmHeadQ8Ptr = q2kPtr;
            weights.LmHeadQuantTypeOverlay = QuantizationType.Q2_K;
        }
        ...
        for (int i = 0; i < numLayers; i++)
        {
            ...
            // in_proj [d_in_proj, hidden]  — contraction axis = hidden
            {
                ...
                nint q2kPtr = QuantizeF32ToQ2K(srcF32, rows, cols);
                nint freshF32 = AllocAlignedF32(elems);
                DequantizeQ2KRowsToF32Span(q2kPtr, rows, cols,
                    new Span<float>((void*)freshF32, checked((int)elems)));
                ...
                ov.InProjQ8Ptr = q2kPtr;
                ov.InProjQuantTypeOverlay = QuantizationType.Q2_K;
            }
            ...
            // out_proj [hidden, d_inner]  — contraction axis = d_inner
            {
                ...
                ov.OutProjQ8Ptr = q2kPtr;
                ov.OutProjQuantTypeOverlay = QuantizationType.Q2_K;
            }
    ...
    /// matrix to Q2_K row-stride bytes via <see cref="Q2KFixture.QuantizeRows"/>.
    /// Returns an aligned native allocation that the test owns (freed in Dispose).
    /// <paramref name="cols"/> must be a multiple of the Q2_K super-block size (256).
    /// </summary>
    private unsafe nint QuantizeF32ToQ2K(ReadOnlySpan<float> src, int rows, int cols)
    {
        if ((cols % 256) != 0)
            throw new InvalidOperationException(
                $"Q2_K quantisation requires cols % 256 == 0 (got cols={cols}). Bump fixture dims.");
        // Materialise the source as a managed array — the fixture quantiser takes one.
        float[] arr = new float[(long)rows * cols];
        src.CopyTo(arr);
        byte[] q2kBytes = Q2KFixture.QuantizeRows(arr, rows, cols);

        long totalBytes = q2kBytes.Length;
        nint dst = (nint)NativeMemory.AlignedAlloc((nuint)totalBytes, 64);
        _q2kAllocs.Add(dst);
        new ReadOnlySpan<byte>(q2kBytes).CopyTo(new Span<byte>((void*)dst, checked((int)totalBytes)));
        return dst;
    }

    /// <summary>
    /// Dequantises Q2_K row-stride bytes back into F32 row-major elements at
    /// <paramref name="dst"/>. Used so the CPU oracle consumes the (lossy) dequantised
    /// values matching what the Vulkan side computes via the Q2_K kernels.
    /// </summary>
    private static unsafe void DequantizeQ2KRowsToF32Span(nint q2kPtr, int rows, int cols, Span<float> dst)
    {
        long rowBytes = Dequantize.RowByteSize(cols, QuantizationType.Q2_K);
        for (int row = 0; row < rows; row++)
        {
            nint rowSrc = q2kPtr + (nint)((long)row * rowBytes);
            Dequantize.ToFloat32(rowSrc, cols, QuantizationType.Q2_K, dst.Slice(row * cols, cols));
        }
    }
