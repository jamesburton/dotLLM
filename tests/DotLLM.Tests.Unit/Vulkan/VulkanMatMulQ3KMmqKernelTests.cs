...
/// <summary>
/// Tolerance + argmax parity test for the dp4a Q3_K MMQ prefill GEMM:
/// <c>QuantizeQ8_1RowsKernel</c> (F32 B[N,K] → Q8_1 row-wise) followed by
/// <c>MatMulQ3KMmqKernel</c> (integer-dot GEMM against Q3_K weights — signed
/// 3-bit (q−4) from qs+hmask with a per-16 signed scale, no min term). Compared
/// against the byte-identical CPU F32 oracle <see cref="Q3KFixture.CpuGemmQ3K"/>.
/// </summary>
[Trait("Category", "GPU")]
[Collection("VulkanKernels")]
public class VulkanMatMulQ3KMmqKernelTests
{
    ...
    public void Mmq_MatchesF32Oracle_ArgmaxAndTolerance(int n, int m, int k)
    {
        ...
        using var mmq = MatMulQ3KMmqKernel.TryCreate(device, spvDir)
            ?? throw new Xunit.Sdk.XunitException("matmul_q3_k_mmq.spv missing or unsupported.");

        var rng = new Random(0x3C + n * 13 + m * 7 + k * 3);
