...
public class VulkanRopeF32KernelTests
{
    ...
    // (the kernel default is the STANDARD ropeDim/2 — see _StandardNeoX below).
    ...
    //
    // DISCRIMINATING / regression guard: the kernel previously hardcoded the NeoX
    // pairing offset to headDim/2, which silently mis-paired every partial-rotary
    // model where ropeDim < headDim (Qwen3MoeHybrid IQ3 forward parity failure).
    // With ropeDim < headDim and headDim/2 != ropeDim/2 these shapes produce a
    // grossly wrong result under the old offset and a bit-exact result under the
    // fix — so this theory is RED before the fix and GREEN after.
    // (seqLen, numHeads, numKvHeads, headDim, ropeDim, theta)
        ...
        int seqLen, int numHeads, int numKvHeads, int headDim, int ropeDim, float theta)
    {
        ...
        // ropeDim/2 freq entries per position, exponent 2*pair/ropeDim — matches
        // the kernel's per-thread freq reconstruction AND the CPU RoPE.Execute
        // partial-rotary path (which slices each head to length ropeDim).
