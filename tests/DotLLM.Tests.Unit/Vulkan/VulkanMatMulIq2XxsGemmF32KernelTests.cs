...
public class VulkanMatMulIq2XxsGemmF32KernelTests
{
    ...
    public void Launch_MatchesCpuReference(int m, int k, int n)
    {
        ...
        var rng = new Random(unchecked((int)0xC0DE_F211) + m * 7 + k * 11 + n);
        ...
        float[] expected = Iq2Fixture.CpuGemmIq2Xxs(weightsIq2, inputB, m, k, n);

        using var device = VulkanDevice.Create();
        using var kernel = MatMulIq2XxsGemmF32Kernel.Create(device, spvDir);
        ...
        Iq2Fixture.AssertClose(expected, actual, $"iq2_xxs GEMM m={m} k={k} n={n}", AbsTol, RelTol);
    }
}
