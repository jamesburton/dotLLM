...
/// <summary>
/// Test-only Q4_K fixture helpers shared by the GEMV / GEMM kernel tests and
/// the model-level Q4_K parity tests.
/// </summary>
/// <remarks>
/// <para>
/// Why this exists: the production CPU side has full Q4_K dequantise + matmul
/// kernels (see <c>DotLLM.Cpu.Kernels.Dequantize.DequantizeQ4_K</c>) but no
/// F32-to-Q4_K quantiser — production loaders only ingest pre-quantised GGUF
/// data. For Vulkan parity we need to round-trip arbitrary FP32 fixtures into
/// Q4_K bytes, so this file contains a minimal scalar quantiser that produces
/// the same byte layout as llama.cpp's <c>quantize_row_q4_K</c>: 256-element
/// super-blocks of 144 bytes laid out as
/// <c>[d:fp16][dmin:fp16][scales[12]][qs[128]]</c> with the 8x6-bit
/// scales/mins packed via the inverse of
/// <c>DequantizeKQuants.UnpackQ4Q5Scales</c>.
/// </para>
/// <para>
/// The fixture quantiser is intentionally NOT placed in <c>DotLLM.Cpu</c> —
/// production code does not need to write Q4_K, and shipping a quantiser
/// without proper amax/grid-search optimisation would invite misuse.
/// </para>
/// </remarks>
internal static unsafe class Q4KFixture
{
    ...
    /// <summary>
    /// Quantise an <c>[m, k]</c> row-major FP32 matrix to Q4_K bytes (one
    /// 144-byte super-block per 256 elements per row). Layout matches
    ...
    /// step = <c>(max_j - min_j) / 15</c>. The 8 per-sub-block (scale, min)
    ...
    /// needs to do. The CPU oracle <c>DequantizeQ4_KScalar</c> is the
    ...
    public static byte[] QuantizeRows(float[] src, int m, int k)
    {
        ...
        Span<byte> nibbles = stackalloc byte[Q4KGroupSize];
        ...
        fixed (byte* dstPtr = dst)
        {
            for (int row = 0; row < m; row++)
            {
                ...
                {
                    float* superSrc = rowSrc + b * Q4KGroupSize;
                    byte* superDst = rowDst + b * Q4KBlockBytes;
                    QuantizeSuperBlock(superSrc, superDst, subBlockScales, subBlockMins, nibbles);
                }
            }
        }
        return dst;
    ...
    /// <summary>
    /// Quantises one Q4_K super-block (256 source floats -> 144 destination bytes).
    /// </summary>
    private static unsafe void QuantizeSuperBlock(
        float* srcSuper, byte* dstSuper,
        Span<float> subScales, Span<float> subMins, Span<byte> nibbles)
    {
        ...
        for (int j = 0; j < NumSubBlocks; j++)
        {
            ...
            // Q4_K nibbles are unsigned 0..15. We'll scale (x - min) by step
            // such that step * 15 ≈ range, then encode the per-sub-block min
            // as -dmin*mn (which is added back as -dmin*mn at decode). Storage
            // form is x ≈ d*scale_j*nibble - dmin*min_j; we pre-compute
            // (sc_float, mn_float) such that x_i = sc_float*nibble_i - mn_float.
            float sc = range > 0 ? range / 15.0f : 0.0f;
            // mn_float satisfies x_i ≈ sc * nibble_i - mn_float for the chosen
            // affine fit; with nibble_i = round((x_i - minV) / sc) we have
            // x_i ≈ sc * nibble_i + minV, hence mn_float = -minV.
            float mn = -minV;
            subScales[j] = sc;
            subMins[j] = mn;
        }
        ...
        //   (sc_used, mn_used) from the rounded values for the nibble quantisation
        //   pass. This avoids drift between encoder and decoder when sc/mn cannot be
        ...
        // ── Pass 4: quantise each element to a 4-bit unsigned nibble using
        //   (sc_used, mn_used). Nibble target: round((x + mn_used) / sc_used). ──
        for (int j = 0; j < NumSubBlocks; j++)
        {
            float sc = scUsed[j];
            float mn = mnUsed[j];
            ...
            for (int i = 0; i < SubBlockSize; i++)
            {
                ...
                if (sc > 0)
                {
                    ...
                    nib = Math.Clamp(v, 0, 15);
                }
                else
                {
                    nib = 0;
                }
                nibbles[outBase + i] = (byte)nib;
            }
        }
        ...
        // DequantizeKQuants.UnpackQ4Q5Scales / llama.cpp's get_scale_min_k4.
        // Sub-blocks 0..3:
        ...
        for (int j = 0; j < 4; j++)
        {
            // Initialise with low 6 bits + top 2 bits of the j+4 partner.
        ...
        for (int j = 4; j < 8; j++)
        {
            // bytes [j+4] in [8..11]: low 4 bits scale partner, high 4 bits min partner.
            byte sLow4 = (byte)(sc6[j] & 0xF);
            byte mLow4 = (byte)(mn6[j] & 0xF);
            scalesPtr[j + 4] = (byte)(sLow4 | (mLow4 << 4));
        }

        // qs[]: pack low/high nibbles per (sub-block-pair, position).
        ...
        // 2p+1 goes into the high nibble.
        ...
        for (int p = 0; p < 4; p++)
        {
            ...
            for (int i = 0; i < SubBlockSize; i++)
            {
                byte lo = nibbles[evenBase + i];
                byte hi = nibbles[oddBase + i];
                qsPtr[p * SubBlockSize + i] = (byte)((lo & 0xF) | ((hi & 0xF) << 4));
            }
        }
    }
    ...
    /// expectations for Q4_K (~5% of dynamic range). This is a structural
    /// safeguard so that later kernel-output asserts are testing the kernel,
    /// not a broken fixture quantiser.
    /// </summary>
    public static void AssertFixtureRoundtrip(float[] srcF32, byte[] q4kBytes, int m, int k)
    {
        ...
        fixed (float* d = dequant)
        {
            ...
            Dequantize.ToFloat32((nint)p, (long)m * k, QuantizationType.Q4_K,
                new Span<float>(d, m * k));
        }

        // Compute relative L2 drift per row; the fixture quantiser uses a
        // simple affine fit so the per-element error budget is roughly
        // (max - min) / 30 (~3% of the range) plus 6-bit scale rounding noise.
        float maxRel = 0;
        for (int row = 0; row < m; row++)
        {
        ...
        Assert.True(maxRel < 0.10f,
            $"Q4_K fixture round-trip drift too large: maxRel={maxRel:G4} (m={m}, k={k}). " +
            "Fixture quantiser is likely mis-packing the 6-bit scales or the qs[] nibbles.");
    }

    /// <summary>
    /// Scalar CPU GEMV reference: reads the same Q4_K bytes the GPU sees,
    ...
    /// in the test.
    /// </summary>
    public static float[] CpuGemvQ4K(byte[] weightsQ4K, float[] x, int m, int k)
    {
        if ((k % Q4KGroupSize) != 0)
            throw new ArgumentException($"k must be a multiple of {Q4KGroupSize}, got {k}", nameof(k));
    ...
    /// <summary>
    /// Scalar CPU GEMM reference: <c>C[N, M] = B[N, K] @ W_q4k[M, K]^T</c>.
    /// Reads the same Q4_K bytes the GPU sees.
    /// </summary>
    public static float[] CpuGemmQ4K(byte[] weightsQ4K, float[] inputB, int m, int k, int n)
    {
        if ((k % Q4KGroupSize) != 0)
            throw new ArgumentException($"k must be a multiple of {Q4KGroupSize}, got {k}", nameof(k));
        ...
        fixed (byte* wPtr = weightsQ4K)
        {
            for (int t = 0; t < n; t++)
            {
                ...
                for (int row = 0; row < m; row++)
                {
                    ...
                    {
                        byte* block = rowBaseW + bb * Q4KBlockBytes;
                        float d = (float)Unsafe.ReadUnaligned<Half>(block);
                        float dmin = (float)Unsafe.ReadUnaligned<Half>(block + 2);
                        fixed (byte* sc = scBuf)
                        fixed (byte* mn = mnBuf)
