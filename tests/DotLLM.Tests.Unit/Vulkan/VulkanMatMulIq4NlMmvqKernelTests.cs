...
/// <summary>
/// Tolerance + argmax parity for the dp4a IQ4_NL MMVQ decode GEMV (issue #339):
/// Q8_1-quantized activation × IQ4_NL codebook weights, compared to the IQ4_NL-
/// byte-identical CPU oracle <see cref="Iq4Fixture.CpuGemvIq4Nl"/>.
/// </summary>
[Trait("Category", "GPU")]
[Collection("VulkanKernels")]
public class VulkanMatMulIq4NlMmvqKernelTests
{
    ...
    public void Mmvq_MatchesF32Oracle_ArgmaxAndTolerance(int m, int k)
    {
        ...
        using var mmvq = MatMulIq4NlMmvqKernel.TryCreate(device, spvDir)
            ?? throw new Xunit.Sdk.XunitException("matmul_iq4_nl_mmvq.spv missing or unsupported.");

        var rng = new Random(0x4e + m * 7 + k * 11);
        ...
        byte[] wq = Iq4Fixture.QuantizeRowsIq4Nl(weightsF32, m, k);
        Assert.Equal(totalBytes, wq.Length);
        Iq4Fixture.AssertFixtureRoundtripIq4Nl(weightsF32, wq, m, k);
        float[] expected = Iq4Fixture.CpuGemvIq4Nl(wq, x, m, k);
        ...
        Iq4MmvqParity.Assert(expected, actual, m, k, RelTol, "IQ4_NL");
    }
}
