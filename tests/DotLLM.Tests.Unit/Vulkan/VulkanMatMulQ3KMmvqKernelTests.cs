...
/// <summary>
/// Tolerance + argmax parity test for the dp4a Q3_K MMVQ decode GEMV (issue #339):
/// <c>QuantizeQ8_1Kernel</c> (F32 activation → Q8_1) then <c>MatMulQ3KMmvqKernel</c>
/// (integer-dot GEMV against the Q3_K weights), compared to the Q3_K-byte-identical
/// CPU oracle <c>Q3KFixture.CpuGemvQ3K</c> with argmax-exact (near-tie tolerant)
/// + a loose abs/rel tolerance sized to the int8-activation quant drift.
/// </summary>
[Trait("Category", "GPU")]
[Collection("VulkanKernels")]
public class VulkanMatMulQ3KMmvqKernelTests
{
    ...
    public void Mmvq_MatchesF32Oracle_ArgmaxAndTolerance(int m, int k)
    {
        ...
        var rng = new Random(0x3a + m * 7 + k * 11);
        ...
        byte[] weightsQ = Q3KFixture.QuantizeRows(weightsF32, m, k);
        Assert.Equal(totalBytes, weightsQ.Length);

        Q3KFixture.AssertFixtureRoundtrip(weightsF32, weightsQ, m, k);

        float[] expected = Q3KFixture.CpuGemvQ3K(weightsQ, x, m, k);
    ...
    private static void AssertParity(float[] expected, float[] actual, int m, int k)
    {
        ...
        Assert.True(errors == 0,
            $"Q3_K MMVQ drift exceeded tolerance (m={m},k={k}): errors={errors}/{m}, " +
