...
/// <summary>
/// End-to-end parity tests for the Vulkan Mamba-3 forward path with Q5_K_M
/// projection-weight upload — Phase 1 sibling of the Q4_K_M tests
/// (<see cref="VulkanMamba3TransformerModelQ4KForwardTests"/>). This proves the
/// full upload-path + RecordMatmul dispatch pipeline for Q5_K is wired through, on
/// top of the kernel-level parity already covered by
/// <see cref="VulkanMatMulQ5KGemvF32KernelTests"/> /
/// <see cref="VulkanMatMulQ5KGemmF32KernelTests"/>.
/// </summary>
/// <remarks>
/// <para>
/// <b>Scope.</b> The three matmul-target projections (<c>in_proj</c>, <c>out_proj</c>,
/// <c>lm_head</c>) run through the Q5_K path. Token embedding always lands as F32
...
/// data is quantised to Q5_K via <see cref="Q5KFixture.QuantizeRows"/> and the raw
/// 176-byte super-block bytes are pinned to the overlay slots
...
/// <c>*QuantTypeOverlay</c> field declares (Q8_0 / Q4_K / Q5_K in Phase 1). The
/// corresponding F32 pointer slots are replaced with freshly-allocated buffers
/// holding dequantised-from-Q5_K values, so the CPU oracle reads the same lossy
/// values the Vulkan path consumes via the Q5_K kernels.
/// </para>
/// <para>
/// <b>Dimensions.</b> All Q5_K contraction axes (<c>HiddenSize</c> for <c>in_proj</c>
/// and <c>lm_head</c>; <c>DInner = NumHeads * HeadDim</c> for <c>out_proj</c>) are
/// bumped to multiples of 256 — a hard requirement of the Vulkan Q5_K matmul kernels.
/// Tolerance abs 5e-3 / rel 1e-3, matching the Q4_K sibling. Q5_K's 5-bit resolution
/// is twice Q4_K's, so the per-element drift is actually tighter; the tolerance bar
/// is the same because the dominant per-step drift is reduction-order plus 6-bit
/// scale rounding, both independent of per-element bit width.
/// </para>
/// </remarks>
[Trait("Category", "GPU")]
[Collection("VulkanKernels")]
public sealed class VulkanMamba3TransformerModelQ5KForwardTests : IDisposable
{
    // Q5_K-friendly dimensions: HiddenSize must be a multiple of 256 (in_proj / lm_head
    ...
    // Native allocations made by the Q5_K overlay (raw super-block bytes + dequantised-
    // back-to-F32 mirror buffers). Freed in Dispose so memory survives the test methods.
    private readonly List<nint> _q5kAllocs = new();

    public VulkanMamba3TransformerModelQ5KForwardTests()
    {
        _scratch = Path.Combine(Path.GetTempPath(), $"dotllm-m3-q5k-vk-{Guid.NewGuid():N}");
        Directory.CreateDirectory(_scratch);
    }

    public unsafe void Dispose()
    {
        foreach (var p in _q5kAllocs)
    ...
    [SkippableFact]
    public void Forward_Q5K_Prefill_SingleLayer_Siso_MatchesCpuReference()
        => AssertVulkanMatchesCpuSiso(numLayers: 1, seqLen: 4, seed: 7);

    [SkippableFact]
    public void Forward_Q5K_Prefill_MultiLayer_Siso_MatchesCpuReference()
        => AssertVulkanMatchesCpuSiso(numLayers: 2, seqLen: 4, seed: 17);

    [SkippableFact]
    public void Forward_Q5K_Decode_SingleToken_Siso_MatchesCpuReference()
    ...
    private void AssertVulkanMatchesCpuSiso(int numLayers, int seqLen, int seed)
    {
        ...
        string path = Path.Combine(_scratch, $"m3-q5k-L{numLayers}-T{seqLen}-s{seed}.safetensors");
        ...
        float[] cpuLogits;
        {
            ...
            ApplyQ5KOverlayInPlace(weights);
        ...
        for (int c = 0; c < VocabSize; c++)
        {
            ...
            Assert.True(diff <= bar,
                $"Q5_K SISO numLayers={numLayers}, seqLen={seqLen}, col={c}: " +
    ...
    /// <c>in_proj</c>/<c>out_proj</c>) to Q5_K via the test fixture quantiser, attaches
    ...
    /// Q5_K values so the CPU oracle consumes the same effective values as the Vulkan
    /// path. Same approach as the Q4_K sibling
    /// (<see cref="VulkanMamba3TransformerModelQ4KForwardTests"/>).
    /// </summary>
    private unsafe void ApplyQ5KOverlayInPlace(Mamba3Weights weights)
    {
        ...
        // ── lm_head ([vocab, hidden]) ─────────────────────────────────────────
        {
            ...
            nint q5kPtr = QuantizeF32ToQ5K(srcF32, vocab, hidden);
            nint freshF32 = AllocAlignedF32(elems);
            DequantizeQ5KRowsToF32Span(q5kPtr, vocab, hidden,
            ...
            weights.LmHead = new Mamba3TensorHandle(
                ...
                OwnsMemory: false);
            weights.LmHeadQ8Ptr = q5kPtr;
            weights.LmHeadQuantTypeOverlay = QuantizationType.Q5_K;
        }
        ...
        for (int i = 0; i < numLayers; i++)
        {
            ...
            // in_proj [d_in_proj, hidden]  — contraction axis = hidden
            {
                ...
                nint q5kPtr = QuantizeF32ToQ5K(srcF32, rows, cols);
                nint freshF32 = AllocAlignedF32(elems);
                DequantizeQ5KRowsToF32Span(q5kPtr, rows, cols,
                    new Span<float>((void*)freshF32, checked((int)elems)));
                ...
                ov.InProjQ8Ptr = q5kPtr;
                ov.InProjQuantTypeOverlay = QuantizationType.Q5_K;
            }
            ...
            // out_proj [hidden, d_inner]  — contraction axis = d_inner
            {
                ...
                ov.OutProjQ8Ptr = q5kPtr;
                ov.OutProjQuantTypeOverlay = QuantizationType.Q5_K;
            }
    ...
    /// matrix to Q5_K row-stride bytes via <see cref="Q5KFixture.QuantizeRows"/>.
    /// Returns an aligned native allocation that the test owns (freed in Dispose).
    /// <paramref name="cols"/> must be a multiple of the Q5_K super-block size (256).
    /// </summary>
    private unsafe nint QuantizeF32ToQ5K(ReadOnlySpan<float> src, int rows, int cols)
    {
        if ((cols % 256) != 0)
            throw new InvalidOperationException(
                $"Q5_K quantisation requires cols % 256 == 0 (got cols={cols}). Bump fixture dims.");
        // Materialise the source as a managed array — the fixture quantiser takes one.
        float[] arr = new float[(long)rows * cols];
        src.CopyTo(arr);
        byte[] q5kBytes = Q5KFixture.QuantizeRows(arr, rows, cols);

        long totalBytes = q5kBytes.Length;
        nint dst = (nint)NativeMemory.AlignedAlloc((nuint)totalBytes, 64);
        _q5kAllocs.Add(dst);
        new ReadOnlySpan<byte>(q5kBytes).CopyTo(new Span<byte>((void*)dst, checked((int)totalBytes)));
        return dst;
    }

    /// <summary>
    /// Dequantises Q5_K row-stride bytes back into F32 row-major elements at
    /// <paramref name="dst"/>. Used so the CPU oracle consumes the (lossy) dequantised
    /// values matching what the Vulkan side computes via the Q5_K kernels.
    /// </summary>
    private static unsafe void DequantizeQ5KRowsToF32Span(nint q5kPtr, int rows, int cols, Span<float> dst)
    {
        long rowBytes = Dequantize.RowByteSize(cols, QuantizationType.Q5_K);
        for (int row = 0; row < rows; row++)
        {
            nint rowSrc = q5kPtr + (nint)((long)row * rowBytes);
            Dequantize.ToFloat32(rowSrc, cols, QuantizationType.Q5_K, dst.Slice(row * cols, cols));
        }
    }
