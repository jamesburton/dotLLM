...
/// across MHA / GQA / sliding-window / soft-cap configurations and prompt
/// lengths up to 2048 tokens.
/// </summary>
/// <remarks>
/// ALiBi parity is not exercised here: the F32 dense scaffold parent does not
/// yet expose <c>AlibiPositionEncoding.CreateSlopes</c>, so the ALiBi test
/// case is held back until the ALiBi-position-encoding infrastructure lands
/// on this branch. The shader still implements ALiBi (gated by the kernel's
/// <c>useAlibi</c> push-constant); the regression coverage will switch on in
/// the follow-up PR.
/// </remarks>
[Trait("Category", "GPU")]
...
public class VulkanFlashAttentionF32KernelTests
{
    ...
    private static void RunOne(int seqQ, int seqKv, int numHeads, int numKvHeads, int headDim,
        int positionOffset, int slidingWindow = 0, float softCap = 0.0f)
    {
        ...
        kernel.Launch(bufQ, bufK, bufV, bufOut,
            ...
            useAlibi: false, softCap: softCap);
    ...
    private static void ComputeExpected(
        ...
        int positionOffset, int slidingWindow, float softCap)
    {
        if (softCap <= 0f)
        {
