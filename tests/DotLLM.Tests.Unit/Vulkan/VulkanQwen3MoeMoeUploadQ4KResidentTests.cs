...
public sealed unsafe class VulkanQwen3MoeMoeUploadQ4KResidentTests : IDisposable
{
        ...
        int n, int numExperts, int interm, int hidden, int activeExperts)
    {
        ...
        Assert.Equal(QuantizationType.Q4_K, residentBundle.BankQuantType);
    ...
    [SkippableFact]
    public void ResidentQuant_FallsBackToF32_WhenBanksAreMixedQuant()
    {
        // A mixed-quant layer (e.g. a hand-edited UD-style GGUF with Q4_K
        // W1/W3 but Q6_K W2) must fall back to F32 — the dispatcher only
        // carries one bank kernel per bundle, and this issue explicitly
        // does not attempt mixed-bank-quant dispatch.
        VulkanMatMulF32KernelTests.SkipIfUnavailable(out string spvDir);
