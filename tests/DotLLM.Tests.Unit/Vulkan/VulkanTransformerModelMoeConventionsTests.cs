...
/// <summary>
/// End-to-end Vulkan parity test for the Qwen-MoE / Phi-3.5-MoE per-expert
/// MoE tensor-naming convention with a distinct <c>moe_intermediate_size</c>
/// (not already covered by <see cref="VulkanTransformerModelMoeForwardTests"/>).
/// The CPU loader's <c>LoadQwenMoeLayer</c> unpacks the per-expert tensors
/// into the standard <c>MoeLayerWeights.W1[] / W2[] / W3[]</c> per-expert
/// pointer arrays before they reach the Vulkan upload code, so the Vulkan
/// path SHOULD be agnostic to the source naming. This test proves that.
/// </summary>
/// <remarks>
/// The Granite-MoE fused-experts convention parity case from the
/// originating commit is intentionally excluded — Granite-MoE
/// (<c>Architecture.GraniteMoe</c> + the Granite safetensors loader) lands
/// in a separate sub-chain (issue/198) that is not part of this PR's base
/// (issue/219 + issue/194). When the Granite-MoE base merges, a follow-up
/// can add the Granite parity case in the same shape.
/// </remarks>
/// <remarks>
/// <para>
