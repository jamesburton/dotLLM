...
/// <summary>
/// Parity tests for the IQ3_S GEMV + GEMM Vulkan kernels against the CPU
/// reference (dequant -> matmul). Tolerance is 5e-3 abs / 2e-3 rel.
/// </summary>
[Trait("Category", "GPU")]
[Collection("VulkanKernels")]
public class VulkanMatMulIq3SF32KernelTests
{
    ...
    public void Gemv_MatchesCpuReference(int m, int k)
    {
        ...
        var rng = new Random(0x3B_2A_F4 ^ (m * 13 + k));
        float[] xF32     = Iq3Fixture.RandomFloats(rng, k, 0.5f);
        float[] weightsF = Iq3Fixture.RandomFloats(rng, m * k, 0.05f);
        byte[] weightsIq = Iq3Fixture.QuantizeRowsIq3S(weightsF, m, k);
        Iq3Fixture.AssertFixtureRoundtripIq3S(weightsF, weightsIq, m, k);

        float[] expected = Iq3Fixture.CpuGemvIq3S(weightsIq, xF32, m, k);

        using var device = VulkanDevice.Create();
        using var kernel = MatMulIq3SGemvF32Kernel.Create(device, spvDir);
        ...
        Iq3Fixture.AssertClose(expected, actual, $"iq3_s gemv m={m} k={k}",
    ...
    public void Gemm_MatchesCpuReference(int m, int k, int n)
    {
        ...
        var rng = new Random(0x77_5D_8A ^ (m * 13 + k * 7 + n));
        float[] inputB    = Iq3Fixture.RandomFloats(rng, n * k, 0.5f);
        float[] weightsF  = Iq3Fixture.RandomFloats(rng, m * k, 0.05f);
        byte[] weightsIq  = Iq3Fixture.QuantizeRowsIq3S(weightsF, m, k);
        Iq3Fixture.AssertFixtureRoundtripIq3S(weightsF, weightsIq, m, k);

        float[] expected = Iq3Fixture.CpuGemmIq3S(weightsIq, inputB, m, k, n);

        using var device = VulkanDevice.Create();
        using var kernel = MatMulIq3SGemmF32Kernel.Create(device, spvDir);
        ...
        Iq3Fixture.AssertClose(expected, actual, $"iq3_s gemm m={m} k={k} n={n}",
            absTol: AbsTol, relTol: RelTol);
    }
}
