...
/// <summary>
/// Tolerance + argmax parity for the dp4a IQ3_S MMVQ decode GEMV (issue #339):
/// Q8_1-quantized activation × IQ3_S codebook-grid weights (two 4-byte grid
/// lookups per pair, 9-bit index, explicit per-pair sign mask, sign·grid → int8 →
/// dp4a), compared to the IQ3_S-byte-identical CPU oracle
/// <see cref="Iq3Fixture.CpuGemvIq3S"/>.
/// </summary>
[Trait("Category", "GPU")]
[Collection("VulkanKernels")]
public class VulkanMatMulIq3SMmvqKernelTests
{
    private const int GroupSize = 256;
    private const int BlockBytes = 110;
    ...
    public void Mmvq_MatchesF32Oracle_ArgmaxAndTolerance(int m, int k)
    {
        ...
        using var mmvq = MatMulIq3SMmvqKernel.TryCreate(device, spvDir, codebooks)
            ?? throw new Xunit.Sdk.XunitException("matmul_iq3_s_mmvq.spv missing or unsupported.");

        var rng = new Random(0x35 + m * 7 + k * 11);
        ...
        byte[] wq = Iq3Fixture.QuantizeRowsIq3S(weightsF32, m, k);
        Assert.Equal(totalBytes, wq.Length);
        Iq3Fixture.AssertFixtureRoundtripIq3S(weightsF32, wq, m, k);
        float[] expected = Iq3Fixture.CpuGemvIq3S(wq, x, m, k);
        ...
        Iq4MmvqParity.Assert(expected, actual, m, k, RelTol, "IQ3_S");
    }
}
