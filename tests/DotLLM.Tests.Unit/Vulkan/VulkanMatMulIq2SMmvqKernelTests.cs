...
/// <summary>
/// Tolerance + argmax parity for the dp4a IQ2_S MMVQ decode GEMV (issue #339):
/// Q8_1-quantized activation × IQ2_S codebook-grid weights (sign·grid → int8 →
/// dp4a, explicit per-pair sign mask), compared to the IQ2_S-byte-identical CPU
/// oracle <see cref="Iq2Fixture.CpuGemvIq2S"/>.
/// </summary>
[Trait("Category", "GPU")]
[Collection("VulkanKernels")]
public class VulkanMatMulIq2SMmvqKernelTests
{
    private const int GroupSize = 256;
    private const int BlockBytes = 82;
    ...
    public void Mmvq_MatchesF32Oracle_ArgmaxAndTolerance(int m, int k)
    {
        ...
        using var mmvq = MatMulIq2SMmvqKernel.TryCreate(device, spvDir, codebooks)
            ?? throw new Xunit.Sdk.XunitException("matmul_iq2_s_mmvq.spv missing or unsupported.");

        var rng = new Random(0x25 + m * 7 + k * 11);
        ...
        byte[] wq = Iq2Fixture.QuantizeRowsIq2S(weightsF32, m, k);
        Assert.Equal(totalBytes, wq.Length);
        Iq2Fixture.AssertFixtureRoundtripIq2S(weightsF32, wq, m, k);
        float[] expected = Iq2Fixture.CpuGemvIq2S(wq, x, m, k);
        ...
        Iq4MmvqParity.Assert(expected, actual, m, k, RelTol, "IQ2_S");
    }
}
