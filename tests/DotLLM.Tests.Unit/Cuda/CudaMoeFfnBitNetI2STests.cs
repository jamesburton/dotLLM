...
public sealed class CudaMoeFfnBitNetI2STests : IDisposable
{
    ...
    {
        // seqLen > 1 routes multiple tokens to the same expert. Originally exercised the
        // per-row I2_S GEMV loop (issue #246 scope: no batched GEMM that pass) — since issue
        // #250 this now exercises the batched I2_S GEMM kernel by default (CudaMoeFfn's
        // UseI2SBatchedGemm defaults to on when the kernel is loaded). See
        // CudaMoeFfnBitNetI2SBatchedGemmTests for the dedicated three-way (CPU oracle / GPU
        // per-row-loop / GPU batched-GEMM) parity gate that exercises both GPU paths explicitly
        // within one process.
        Skip.IfNot(CudaDevice.IsAvailable(), "No CUDA GPU available");
        Skip.If(_kernels == null, "PTX files not found");
        Skip.IfNot(_kernels!.HasBitNetMoeKernels, "BitNet-MoE CUDA kernels not available");
