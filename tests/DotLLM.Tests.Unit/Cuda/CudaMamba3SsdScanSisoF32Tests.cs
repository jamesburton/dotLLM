...
/// compiled -fmad=false (native/kernels/mamba3_ssd_scan_siso_f32.cu's own header documents
/// this). <c>decay = expf(adt[...])</c> feeds the state recurrence
/// (<c>s = decay * state + vp * kScaled</c>) at every token, and CUDA's precise libdevice
/// <c>expf</c> is not guaranteed bit-identical to .NET's <c>MathF.Exp</c> for the same
/// input (IEEE 754 does not mandate correctly-rounded transcendentals) — the same class of
/// drift documented on <see cref="CudaKernels"/>'s GDN kernel block and on
/// <c>CudaMamba3DataRopeF32Tests</c>. Because this kernel folds the ENTIRE sequence into
/// one launch with a sequential per-token state update, any sub-ULP drift introduced at
/// t=0 compounds through every later token's state and y — so the tolerance here is looser
/// than Task 2's data-RoPE kernel (which only chains a cumulative-angle sum, not a full
/// decayed matmul-like recurrence). Measured on this box (RTX 3060): worst observed
/// maxAbsDiff across all three shapes/gate combinations was 1.19e-6 (the
/// nHead=32/headDim=64/dState=128/seqLen=6/hasZ=true case), ~84x smaller than the 1e-4
/// tolerance below — still ~5 orders of magnitude tighter than a real transcription bug
/// (wrong stride/index/broadcast/accumulator-order) would produce, which shows up as
/// O(0.1+).
/// </remarks>
[Trait("Category", "GPU")]
public class CudaMamba3SsdScanSisoF32Tests
{
        ...
        int nHead, int headDim, int dState, int seqLen, bool hasZ)
    {
        ...
        try
        {
            ...
            _out.WriteLine($"nHead={nHead} headDim={headDim} dState={dState} seqLen={seqLen} hasZ={hasZ}: " +
