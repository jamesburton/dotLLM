...
- generated: 2026-09-21T10:23:25+0100
- commit: `3f372129`
...
| `openai/usage.stream.unrequested` | openai | no usage when include_usage is absent | **FAIL** | #450 | AssertionError: 1 chunk(s) carried usage although include_usage was not requested (indices [8]) |
| `openai/models.list` | openai | model list | **PASS** | — | 1 model(s), first=Llama-3.2-1B-Instruct-GGUF |
| `openai/models.retrieve` | openai | model retrieve | **NOT-IMPL** | #450 | 404, empty body — route not registered |
| `openai/embeddings` | openai | embeddings | **NOT-IMPL** | #451 | 404, empty body — route not registered |
...
| `anthropic/messages` | anthropic | plain completion | **NOT-IMPL** | #448 | 404, empty body — route not registered |
| `anthropic/messages.stream` | anthropic | streaming (SSE event types) | **NOT-IMPL** | #448/#449 | 404, empty body — route not registered |
| `anthropic/tool.single` | anthropic | tool call (single, forced) | **NOT-IMPL** | #448 | 404, empty body — route not registered |
| `anthropic/structured` | anthropic | structured output (forced tool) | **NOT-IMPL** | #448 | 404, empty body — route not registered |
| `anthropic/count_tokens` | anthropic | token counting via count_tokens | **NOT-IMPL** | #449 | 404, empty body — route not registered |
| `anthropic/usage` | anthropic | usage on the message response | **NOT-IMPL** | #448 | 404, empty body — route not registered |
...
**Totals:** PASS=6, FAIL=6, NOT-IMPL=8, NOT-EXERCISED=2
