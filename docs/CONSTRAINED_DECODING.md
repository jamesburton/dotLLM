...
Guarantees output matches specific schema: required fields, types, enum values, nested structures, and string length bounds (`maxLength`/`minLength` — the closing quote is forced at `maxLength`, so a value always terminates). Schema compiled into automaton enforcing structural constraints. **Highest value** for tool calling and structured APIs.

**Implementation** (`JsonSchemaConstraint`): Layers `SchemaTracker` on top of `JsonCharParser`. `SchemaCompiler` compiles JSON Schema to a flat `SchemaNode[]` array + `PropertyNameTrie[]` for key/enum validation. Each tracked position is a `BranchState` (~1.3KB) using `InlineArray` stacks for node indices, emitted-property bitmasks, key char buffer, and array indices. To enforce `anyOf` exactly (#104 — needed for multi-tool calls), `SchemaTracker` holds up to `MaxParallelBranches = 8` `BranchState`s and narrows them in lockstep: a char is allowed if any live branch allows it; branches are pruned (by the *pre-advance* decision, so closing quotes and structural divergence prune correctly) as the input disambiguates. Schemas with no `anyOf` never fork, so a **single-branch fast path** clones just one `BranchState` (~1.3KB) on the per-token hot path — byte-identical to the pre-#104 behavior — while only multi-branch (`anyOf`) states pay the wider clone. The emitted-property trie also prunes already-emitted keys (`ReachableTerminalBits`) so the model can never enter a dead-end key prefix (empty mask). Composite state key: `(parserKey, nodeIdx, emittedProps, triePos)` for the single-branch case, folded across live branches otherwise. Cache: `Dictionary<SchemaStateKey, TokenMask>` (LRU eviction).
