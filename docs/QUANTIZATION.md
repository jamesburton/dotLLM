...
**Memory is the load-bearing constraint, not correctness.** Every Mach-1 tensor must be fully trellis-decoded before it is usable (there is no "raw quant bytes, dequant per row on demand" shortcut the way GGUF's K-quants have) — decoding all 256 experts × 40 layers × 3 projections to F32 is ≈128 GB resident, and even bf16 would be ≈64 GB, both far beyond a typical workstation. `LoadFromMach1Packed` decodes layer files sequentially (one open at a time) but the *unmodified* forward pass still requires every layer's weights present as plain pointers once loading completes, so there is currently no way to run the real 40-layer checkpoint end-to-end on hardware below roughly that RAM ceiling — full-scale resident-memory avoidance is Phase C's fused-kernel path (computing the GEMV directly in the rotated/trellis domain), not this loader. Phase B's test coverage therefore validates the loader against a config **truncated to a handful of layers** (one full `full_attention_interval` period — 3 GDN + 1 full-attention — exercises both token-mixing paths at ~3 GB/layer) rather than attempting a full-model load.

**Not yet done** (tracked by issue #266's Phase C): the fused additive-domain GEMV (`y = diag(sv)·H_m·[wave_gamma⊙Wunit]·H_n·diag(su)·x`) on CPU/CUDA/Vulkan, which is the actual point of the codec — it lets the routed-expert weights stay resident in ~7.5 GB of packed bytes instead of ~128 GB of dense fp32. Full 40-layer end-to-end generation, top-1 token agreement vs. the base GGUF, and perplexity delta are consequently also not yet measured — they require either Phase C's fused path or a machine with dramatically more RAM than dense-decode-to-existing-forward-pass demands.
