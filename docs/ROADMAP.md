...
| 60d | **Mamba-3 pooled forward scratch (`Mamba3ForwardScratch`)** :white_check_mark: | Eliminate per-Forward-call managed allocations on the Mamba-3 hot path. New `Mamba3ForwardScratch : IDisposable` (mirrors `NemotronHForwardState`) owns 13 named 64-byte-aligned `NativeMemory.AlignedAlloc` buffers (`Proj`, `X`, `Z`, `Dt`, `Adt`, `Trap`, `Gamma`, `Scale`, `AnglesRaw`, `B`, `C`, `QkPreDot`, `YScan`) with power-of-two `EnsureCapacity(seqLen)` growth and lazy first-call init. `Mamba3Block.Forward` / `ForwardMimo` take the scratch as a required first parameter; `Mamba3TransformerModel` owns one instance and threads it through every layer of every Forward call (48 layers × ~10 buffers/call → 0 allocations/step on the 370M checkpoint, vs ≈480 managed-float[] allocations before). Scratch sizing includes both SISO and MIMO B/C widths (backed by `max(1, mimoRank) · nHead · dState`), and `ComputeMemoryBytes` reports the scratch footprint. Numerics preserved bit-exact. 9 new unit tests exercise scratch lifecycle: zero-capacity lazy init, power-of-two growth, no-op when EnsureCapacity(≤cap), accessor bounds check, idempotent Dispose, accessor-after-Dispose throws. | 60, 60a |
| 60e | **Mamba-3 streaming SISO decode (chunk-boundary state)** :white_check_mark: | Closes the canonical `shifted_γ[t] = DT[t+1]·(1-trap[t+1])` 1-token-lookahead chunk-edge drift documented under 60a. `Mamba3State` gains two new persistent buffers per layer — `k_state [n_head, d_state]` (previous chunk's last-token post-RoPE, pre-scale K) and `v_state [n_head, head_dim]` (previous chunk's last-token V = x) — matching canonical `mamba3.py:142` / `mamba3_siso_fwd.py:341-352` / `mamba3_siso_step_kernel`. `Mamba3Block.Forward` gains optional `kState` / `vState` span parameters: at chunk START it folds `ssm_state += v_state · k_state · DT[0] · (1 - trap[0])` into the SSM state BEFORE the scan (exactly the deferred boundary term a one-shot forward produces via `scale[T_prev-1] = γ + shifted_γ`); at chunk END it persists this chunk's `bHRN[T-1]` → `kState` and `xBuf[T-1]` → `vState`. `Mamba3TransformerModel.Forward(..., Mamba3State)` threads all four buffers through every layer. Result: on the real `ib-ssm/mamba3-370M-10BT` (48 layers, 32 heads, `d_state=128`) the last-token logit drift max_abs drops from ≈6.5 to ≈3.3 and the top-1 argmax now matches between one-shot prefill and 2-prefill+1-decode (previously differed). On tiny synthetic fixtures prefill+decode reproduces one-shot to F32-reorder noise (~1e-8 on 2+2 splits, ~2e-6 on three-way splits). Tightened assertions: `PrefillThenDecode_ApproximatesOneShot_WithExpectedDrift` → `PrefillThenDecode_BitEqualsOneShot` (abs_tol 5e-5→1e-5, three schedules); `DecodeMatchesPrefillOnRealCheckpoint` ceiling 100→10 with new `Assert.Equal(argmaxPrefill, argmaxSplit)`. New `Mamba3BlockStreamingTests` (7 split schedules × {y, ssm_state, k_state, v_state}). MIMO streaming deferred — canonical's tilelang MIMO kernel has no input-state signature. One-shot call sites unchanged (overload without kState/vState preserved). | 60, 60a, 60d |
...
| **Paged attention kernels** | True PagedAttention: attention kernels that read KV blocks directly via block table indirection, eliminating the staging-buffer gather from step 36. Completes the full PagedAttention story (step 36 is the memory management half, this is the kernel half). Significant throughput improvement for long sequences by avoiding the O(seqLen × headDim) gather copy per layer. | Extends paged KV-cache (36). |
