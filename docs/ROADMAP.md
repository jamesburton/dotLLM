...
| 37 | **Prompt caching (advanced)** :white_check_mark: | Automatic cross-request prefix sharing via trie of computed KV blocks. Reference-counted shared blocks. LRU eviction (zero-refcount only — active-sequence preemption is Step 59). Optional explicit `prefix_id` API via `/v1/prompt-cache/{id}` plus an optional `prefix_id` field on chat/completion requests. Upgrades simple prompt caching (step 54) to work with paged KV-cache. Hard dependency on block-based allocation (36) for cross-sequence block sharing. Only benefits multi-user serving (shared system prompts across concurrent requests). | 36 |
