...
| Q4_K batched-MMQ (prefill, DISABLED by default) | `quantized_gemv_mmq.cu` | `quantized_gemv_q4_k_mmq_batched` | dp4a, MMQ_ROWS_PER_BLOCK(4) × MMQ_BATCH_M_TILE(2) warps/block; numerically correct (issue #349) but measured 2x-51x slower than dequant→cuBLAS at every tested prefill length — narrow M-tile means weight reads aren't amortized across the batch. Disabled via `MmqBatchedMinSeqLen=int.MaxValue`; see #367 for the redesign needed before re-enabling |
...
| Pre-Q8_1 input quant (batched) | `quantize_x.cu` | `quantize_x_to_q8_1_batched` | Companion kernel for the above — quantizes all seqLen activation rows in one 2D-grid launch |
...
GEMM/GEMV for prefill use cuBLAS (`cublasHgemm` / `cublasGemmEx`) directly for every quant type today.
A dp4a-batched-MMQ PTX kernel exists for Q4_K (`quantized_gemv_q4_k_mmq_batched`, issue #349) but is
disabled by default (`CudaKernels.MmqBatchedMinSeqLen=int.MaxValue`) — it is numerically correct but was
measured 2x-51x slower than cuBLAS at every tested prefill length due to a too-narrow per-block M-tile
(see issue #367 for the root-cause analysis and the wider-tile redesign needed before it can be enabled).
...
- **Fused quantized GEMM for prefill**: Marlin-style dequant-in-register. Decode is now MMQ + MMVQ-large + pre-Q8_1 (Qwen3-8B Q4_K_M decode hits 33 tok/s eager on RTX 3060 — inside llama.cpp's reported range); prefill still uses dequant→cuBLAS HGEMM. **Attempted for Q4_K (issue #349, 2026-08-13): built and correctness-verified a dp4a-batched-MMQ prefill kernel (`quantized_gemv_q4_k_mmq_batched`), but it measured 2x-51x SLOWER than the cuBLAS baseline at every tested prefill length (RTX 3060, `-p` 4-512) — the per-block M-tile (2 tokens) is too narrow to amortize weight reads the way cuBLAS's GEMM tiling does, so it ships disabled by default. See issue #367 for the root-cause analysis and the wider-tile (shared-memory weight staging) redesign needed to actually win.**
