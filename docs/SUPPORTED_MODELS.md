...
| DeepSeek-V2 | `Architecture.DeepSeekV2` | HF tokenizer.json (BPE + ByteLevel) | RoPE Norm with optional YaRN (fields stored, not yet applied — see P2.2) | MLA (low-rank Q/KV + decoupled RoPE); KV quant N/A today | yes: routed + multi-shared expert; `first_k_dense_replace` dense prefix | Llama set plus MLA block (`kv_lora_rank`, `qk_nope_head_dim`, `qk_rope_head_dim`, `v_head_dim`, optional `q_lora_rank`), MoE block (`n_routed_experts`, `num_experts_per_tok`, `moe_intermediate_size`, `n_shared_experts`, `first_k_dense_replace`), optional `rope_scaling` | `verified: tiny-random` — `yujiepan/deepseek-v2-tiny-random` (`TinyDeepseekMlaSafetensorsLoadTests.RealDeepseekConfig_IsDetectedAsMlaWithMoe`) plus unit `TransformerModelMlaForwardTests` | Attention routes through `LoadDeepSeekMlaLayer` → `MlaLayerWeights`; scalar `MlaAttention` kernel. No real-weight end-to-end run yet — DeepSeek-V2-Lite checkpoint tracked as P0.2. |
...
`MlpOnlyLayers`. YaRN rope_scaling fields are parsed and stored on
`MlaConfig` but not yet consumed by the attention kernel (PLANS.md P2.2).
The latent KV-cache and `W_q_nope @ W_k_nope^T` absorption that MLA's
production memory win depends on are tracked as P2.3 — today the scalar
kernel reruns the full MLA forward per call.
...
The **MIMO** path is blocked: the MIMO kernel + Block integration is
complete (session step 60f) but `Mamba3TransformerModel.LoadFromSafetensors`
throws `NotSupportedException` when `Mamba3Config.IsMimo = true` — the
remaining work is `Mamba3WeightLoader` picking up `mimo_x` / `mimo_z` /
`mimo_o` tensors and rank-aware B_bias/C_bias. See PLANS.md P0.3. No
public MIMO checkpoint exists so real-weight MIMO verification is
indefinitely deferred (P4.3).
