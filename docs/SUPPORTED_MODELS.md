...
and two-chunk chunk-boundary schedules, max_abs 7.45e-9); a gated real-weight
prefill+decode parity test (`IbSsmMamba3CudaParityTests`, same
`ib-ssm/mamba3-370M-10BT` checkpoint gate as the CPU/Vulkan tests) exists but
has not yet run against real weights (checkpoint download in progress as of
writing) — treat CUDA as real-weight-*pending*, not real-weight-verified,
until that gate reports green. MIMO is synthetic-fixture-only on CUDA, same
