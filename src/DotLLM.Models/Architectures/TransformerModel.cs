...
public sealed unsafe class TransformerModel : IModel
{
                           ...
                           int deviceId, IKvCache? kvCache)
    {
        ...
        for (int layer = 0; layer < numLayers; layer++)
        {
            ...
            // When a LoRA adapter is active we also need the F32 normalised hidden state
            // (normOut) to feed LoraDelta — the fused RmsNormQuantize decode path skips
            // that intermediate. Force the unfused path in both cases.
            bool adapterActive = _currentAdapter is not null;
            if (seqLen == 1 && _threadPool != null && lw.QQuantType != QuantizationType.I2_S && !adapterActive)
            {
            ...
            // When a LoRA adapter is active we also need F32 normOut to feed LoraDelta
            // (same reasoning as the Q/K/V site above).
            bool ffnAdapterActive = _currentAdapter is not null;
            if (seqLen == 1 && _threadPool != null && lw.GateQuantType != QuantizationType.I2_S && !ffnAdapterActive)
            {
                // Decode path: try fused RmsNorm+Quantize (skips normOut intermediate)
