...
#pragma warning restore IDE0052, CA1823
    ...
    private void ValidateAdapterForModel(ILoraAdapter adapter)
    {
        ...
        // kv_b_proj, o_proj) or per-expert (mlp.experts.{j}.{...}) projection
        // names. All three MLA forward kernels — Execute (Phase A, expanded
        // cache), ExecuteLatent (Phase B, absorbed decode), and
        // ExecuteLatentHybrid (Phase C, hybrid expand-prefill / absorbed-
        // decode) — now apply every one of those MLA-specific deltas,
        // including kv_b_proj, which is never expanded as a plain GEMV in
        // the absorbed kernels: its delta is folded into the absorbed
        // Q/K/V math via a low-rank "delta-expansion" over the cached
        // latent (Phase B) or applied directly to the fully-expanded
        // per-position scratch (Phase C's expand-prefill sub-path) — see
        // MlaAttention.ApplyKvBProjLoraDeltaExpanded for the derivation.
        // A target that isn't one of the above names (i.e. not yet emitted
        // by the PEFT loader) is a no-op rather than an error.
    }
