...
/// <summary>
/// Resolves an opened <see cref="ISafetensorsTensorSource"/> against
...
/// <remarks>
/// <para>
/// Accepts any <see cref="ISafetensorsTensorSource"/> implementation —
/// single-file (<see cref="SafetensorsFile"/>) or multi-shard
/// (<see cref="MultiShardSafetensorsFile"/>). This retrofit honours the
/// safetensors-chain caveat on PR #182 that the Mamba-3 chain owns
/// migrating its loaders to the interface.
/// </para>
/// <para>
...
public static class Mamba3WeightLoader
{
    ...
    /// <paramref name="source"/> against the shapes implied by
    ...
    /// <param name="source">An opened safetensors source (single-file or multi-shard). Must outlive the returned weights.</param>
    ...
    public static Mamba3Weights Load(ModelConfig config, ISafetensorsTensorSource source)
    {
        ...
        var emb = ResolveRequired(
            source, Mamba3TensorMapping.TokenEmbedding, [vocab, hidden], entries);

        var finalNorm = ResolveRequired(
            source, Mamba3TensorMapping.FinalNorm, [hidden], entries);
        ...
        if (config.TiedEmbeddings)
        {
            ...
            if (!source.TensorsByName.ContainsKey(Mamba3TensorMapping.LmHead))
            {
            ...
            else
            {
                lmHead = ResolveRequired(
                    source, Mamba3TensorMapping.LmHead, [vocab, hidden], entries);
            }
        }
        else
        {
        ...
        for (int i = 0; i < numLayers; i++)
        {
            var norm = ResolveRequired(
                source, Mamba3TensorMapping.LayerNorm(i), [hidden], entries);
            var inProj = ResolveRequired(
                source, Mamba3TensorMapping.InProj(i), [dInProj, hidden], entries);
            var outProj = ResolveRequired(
                source, Mamba3TensorMapping.OutProj(i), [hidden, dInner], entries);
            var bNorm = ResolveRequired(
                source, Mamba3TensorMapping.BNorm(i), [dState], entries);
            var cNorm = ResolveRequired(
                source, Mamba3TensorMapping.CNorm(i), [dState], entries);
            ...
            var bBias = ResolveRequired(
                source, Mamba3TensorMapping.BBias(i), [numHeads, bcBiasMiddle, dState], entries);
            var cBias = ResolveRequired(
                source, Mamba3TensorMapping.CBias(i), [numHeads, bcBiasMiddle, dState], entries);
            var d = ResolveRequired(
                source, Mamba3TensorMapping.D(i), [numHeads], entries);
            var dtBias = ResolveRequired(
                source, Mamba3TensorMapping.DtBias(i), [numHeads], entries);
            ...
            if (isMimo)
            {
                ...
                mimoX = ResolveRequired(
                    source, Mamba3TensorMapping.MimoX(i), [numHeads, mimoRank, headDim], entries);
                mimoZ = ResolveRequired(
                    source, Mamba3TensorMapping.MimoZ(i), [numHeads, mimoRank, headDim], entries);
                mimoO = ResolveRequired(
                    source, Mamba3TensorMapping.MimoO(i), [numHeads, mimoRank, headDim], entries);
            }
    ...
    {
        if (!source.TensorsByName.TryGetValue(name, out var desc))
        {
        ...
        // Each shard in a multi-shard source has its own mmap base pointer,
        // so we ask the source for the tensor's absolute pointer rather than
        // computing it from a single DataBasePointer.
        nint ptr = source.GetTensorPointer(name);
