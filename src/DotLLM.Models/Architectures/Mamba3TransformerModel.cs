...
#pragma warning restore IDE0052, CA1823
    ...
    /// source — either a single-file <see cref="SafetensorsFile"/> or a
    /// multi-shard <see cref="MultiShardSafetensorsFile"/>. The
    /// <paramref name="source"/> must remain alive for the lifetime of the
    /// returned model; it is held by a GC anchor here, but the caller still
    /// must dispose it after disposing the model.
    /// </summary>
    /// <param name="source">An opened safetensors source positioned at a Mamba-3 checkpoint.</param>
    ...
    /// safetensors source. Caller can introspect
    /// <c>Mamba3WeightLoader.Load(config, source).Report</c> for a pre-check.
    /// </exception>
    public static Mamba3TransformerModel LoadFromSafetensors(ISafetensorsTensorSource source, ModelConfig config)
    {
        ArgumentNullException.ThrowIfNull(source);
        ArgumentNullException.ThrowIfNull(config);

        Mamba3Weights weights = Mamba3WeightLoader.Load(config, source);
        try
        {
            return FromLoadedWeights(config, weights, lifetimeAnchor: source);
        }
        catch
        {
            weights.Dispose();
