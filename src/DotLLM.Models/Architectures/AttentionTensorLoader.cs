...
/// caller's <c>owned</c> list (BF16 upcasts + fused-QKV splits). The struct
/// itself owns nothing — lifetime tracking is unchanged.
/// </para>
/// </remarks>
...
/// Granite-MoE GQA). See PLANS.md §P1.1.
/// </summary>
/// <remarks>
...
internal static class AttentionTensorLoader
{
    ...
    /// </summary>
    /// <param name="variant">Attention layout (GQA separate or Phi-3 fused).</param>
    /// <param name="file">Safetensors source (single-shard or multi-shard view).</param>
    /// <param name="config">Model config — supplies head dims, etc.</param>
    ...
    /// fused-QKV splits) so the parent
