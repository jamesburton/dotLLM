...
/// <summary>
/// Pre-allocated, 64-byte-aligned scratch buffers for the Qwen3HybridDense forward pass
/// (e.g. PrismML's Bonsai-27B). Sized for the widest of the GDN and full-attention sub-layer
/// paths and grown in power-of-two steps by <see cref="EnsureCapacity"/> to keep the hot path
/// allocation-free.
/// </summary>
/// <remarks>
/// Adapted from <see cref="Qwen3MoeHybridForwardState"/>: the shared/GDN/full-attention
/// buffers are identical (same token-mixing paths), but the MoE routing scratch
/// (<c>MoeAssignExpert</c>/<c>MoeBucketCursors</c>/etc.) is replaced with dense SwiGLU FFN
/// scratch (<c>FfnGate</c>/<c>FfnUp</c>/<c>SiluOutput</c>, sized to <c>intermediateSize</c>),
/// matching <c>TransformerModel.cs</c>'s dense FFN buffer naming/shape convention.
/// </remarks>
internal sealed unsafe class Qwen3HybridDenseForwardState : IDisposable
{
    ...
    /// <summary><c>ffn_gate.weight @ normOutput</c>: <c>[seqLen, intermediateSize]</c>.</summary>
    public nint FfnGate;

    /// <summary><c>ffn_up.weight @ normOutput</c>: <c>[seqLen, intermediateSize]</c>.</summary>
    public nint FfnUp;
    ...
    public Qwen3HybridDenseForwardState(
        int hiddenSize,
        int vocabSize,
        int qElems,
        int kvElems,
    ...
    public void EnsureCapacity(int seqLen)
    {
        ...
        FfnGate = AllocFloats((long)cap * _intermediateSize);
