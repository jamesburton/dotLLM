...
public static class MessagesEndpoint
{
        ...
        HttpContext httpContext)
    {
        ...
        // Resolve LoRA adapter (dotLLM extension) — bad name → 400 with available list.
        ...
        {
            await WriteErrorAsync(httpContext, 400, "invalid_request_error", ex.Message);
            return;
        }
        ...
        if (request.Stream)
            await HandleStreamingAsync(request, generator, state, httpContext, prompt, options,
                messageId, modelId, tools, adapter, promptTokenCount, ct);
        else
            await HandleNonStreamingAsync(request, generator, state, httpContext, prompt, options,
                messageId, modelId, tools, adapter, ct);
    }
        ...
        CancellationToken ct)
    {
        ...
        bool matchedStopSequence = false;
        if (finishReason == FinishReason.Stop)
            matchedStopSequence = MatchStopSequence(text, request.StopSequences) is not null;

        string stopReason = AnthropicConverter.ToStopReason(finishReason, matchedStopSequence);
        string? stopSequence = stopReason == "stop_sequence"
            ? MatchStopSequence(text, request.StopSequences)
            : null;
