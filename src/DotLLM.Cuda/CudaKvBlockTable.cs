...
/// <summary>
/// Per-sequence mapping from logical token positions to physical block IDs in a
/// <see cref="CudaKvBlockPool"/>. Mirrors <see cref="DotLLM.Engine.KvCache.KvBlockTable"/>
/// (issue #252) — same host-side bookkeeping shape (a growable <see cref="List{T}"/> of
/// physical block IDs, fork/CoW semantics) — but resolves against a GPU-resident pool, so
/// copy-on-write (<see cref="EnsureWritable"/>) drives a device-to-device copy on a caller-supplied
/// CUDA stream instead of a host <c>Buffer.MemoryCopy</c>.
/// </summary>
public sealed class CudaKvBlockTable
{
    ...
    /// </summary>
    public CudaKvBlockTable(CudaKvBlockPool pool)
    {
        _pool = pool;
    ...
    /// </summary>
    public void Fork(CudaKvBlockTable target)
    {
    ...
    /// <paramref name="tokenCount"/> tokens. Returns blocks that are fully populated only —
    ...
    /// If the block is shared (ref count &gt; 1), copies it (copy-on-write) via a
    /// device-to-device async copy on <paramref name="stream"/>.
    /// </summary>
    /// <remarks>
    /// Assumes single-writer access: the owning sequence controls this table's progression.
    /// There is a theoretical TOCTOU race between <see cref="CudaKvBlockPool.RefCount"/> and
    /// <see cref="CudaKvBlockPool.CopyBlock"/> if another thread releases the block concurrently,
    /// but sequences do not share table mutation — each sequence owns its block table exclusively
    /// (same assumption as the CPU reference).
    /// </remarks>
    public void EnsureWritable(int position, nint stream)
    {
        int logicalBlock = position / _blockSize;
        int blockId = _blockIds[logicalBlock];
        if (_pool.RefCount(blockId) > 1)
        {
            int newBlockId = _pool.CopyBlock(blockId, stream);
