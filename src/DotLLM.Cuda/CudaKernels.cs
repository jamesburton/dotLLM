...
public sealed unsafe class CudaKernels : IDisposable
{
    ...
    private const nuint Half2Alignment = 4;
    private const nuint Float2Alignment = 8;
    ...
    [Conditional("DEBUG")]
    private static void AssertAligned(nint pointer, nuint alignment, string argumentName, string kernelName)
    {
        nuint pointerValue = unchecked((nuint)pointer);
        Debug.Assert(IsAligned(pointer, alignment),
            $"{kernelName}: {argumentName} pointer 0x{pointerValue:X} is not {alignment}-byte aligned; scalar fallback path will be used.");
    }

    private static bool IsAligned(nint pointer, nuint alignment)
    {
        nuint value = unchecked((nuint)pointer);
