...
/// <summary>
/// IQ3_XXS prefill-path batched GEMM: <c>C[N, M] = B[N, K] @ W_iq3xxs[M, K]^T</c>.
/// </summary>
public sealed class MatMulIq3XxsGemmF32Kernel : IDisposable
{
    /// <summary>IQ3_XXS super-block size in bytes.</summary>
    public const int IQ3_XXSBlockBytes = QuantFormat.IQ3_XXSBlockBytes;
    /// <summary>Elements per IQ3_XXS super-block.</summary>
    public const int IQ3_XXSGroupSize = QuantFormat.KQuantGroupSize;
    ...
    private MatMulIq3XxsGemmF32Kernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool,
    ...
    /// <summary>Loads <c>matmul_iq3_xxs_f32_gemm.spv</c>, allocates the shared codebook
    /// SSBOs, and creates the pipeline.</summary>
    public static MatMulIq3XxsGemmF32Kernel Create(VulkanDevice device, string spvDir)
    {
    ...
    internal static MatMulIq3XxsGemmF32Kernel CreateWithCodebooks(VulkanDevice device, string spvDir, Iq3Codebooks codebooks)
        => CreateInternal(device, spvDir, codebooks, ownsCodebooks: false);

    private static MatMulIq3XxsGemmF32Kernel CreateInternal(VulkanDevice device, string spvDir, Iq3Codebooks codebooks, bool ownsCodebooks)
    {
        string path = Path.Combine(spvDir, "matmul_iq3_xxs_f32_gemm.spv");
        if (!File.Exists(path))
        ...
        return new MatMulIq3XxsGemmF32Kernel(device, module, pipeline, pool, codebooks, ownsCodebooks);
    }
        ...
        int m, int k, int n)
    {
        ...
        {
            weights.Handle, inputB.Handle, outputC.Handle, _codebooks.Iq3XxsGrid.Handle, _codebooks.Ksigns.Handle,
        };
