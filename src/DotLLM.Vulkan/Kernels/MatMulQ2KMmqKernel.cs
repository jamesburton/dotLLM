...
/// <summary>
/// Q2_K MMQ prefill-path batched GEMM: <c>C[N, M] = B[N, K] @ W_q2k[M, K]^T</c>
/// via integer dp4a. The activation <c>B</c> must already be Q8_1 row-wise
/// (<see cref="QuantizeQ8_1RowsKernel"/>); this kernel decodes the Q2_K 2-bit quants
/// with a 4-bit per-16 scale/min (asymmetric min term applied via an in-kernel per-16
/// activation sum) into shared memory, runs <c>dotPacked4x8AccSatEXT</c> per 32-element
/// sub-block, and scales the result accordingly.
/// </summary>
/// <remarks>
...
public sealed class MatMulQ2KMmqKernel : IDisposable
{
    ...
    private MatMulQ2KMmqKernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool)
    {
        _device = device;
        _module = module;
        _pipeline = pipeline;
    ...
    /// <summary>
    /// Loads <c>matmul_q2_k_mmq.spv</c> from <paramref name="spvDir"/> and builds the
    ...
    /// <see cref="MatMulQ2KGemmF32Kernel"/>.
    /// </summary>
    public static MatMulQ2KMmqKernel? TryCreate(VulkanDevice device, string spvDir)
    {
        if (!device.HasIntegerDotProduct)
            return null;

        string path = Path.Combine(spvDir, "matmul_q2_k_mmq.spv");
        if (!File.Exists(path))
            return null;
        ...
        return new MatMulQ2KMmqKernel(device, module, pipeline, pool);
    }
    ...
    public void Launch(
        VulkanDevice.Buffer weightsQ2K, VulkanDevice.Buffer xq, VulkanDevice.Buffer xds,
        VulkanDevice.Buffer outputC,
        int m, int k, int n)
    {
        using var ctx = _device.CreateSubmitContext();
        ctx.Begin();
        Record(ctx.CommandBuffer, weightsQ2K, xq, xds, outputC, m, k, n);
        ctx.SubmitAndWait();
    }

    /// <summary>Records the MMQ Q2_K GEMM into <paramref name="cmdBuf"/> without submitting.</summary>
    /// <param name="cmdBuf">Open Vulkan command buffer to append commands to.</param>
    /// <param name="weightsQ2K">Raw Q2_K blob of <c>M * (K / 256) * 84</c> bytes, rows contiguous.</param>
        ...
        int m, int k, int n)
    {
        ...
        if (weightsQ2K.Size < weightsMin)
            throw new ArgumentException(
                $"Weights buffer too small: need >= {weightsMin} bytes, got {weightsQ2K.Size}.", nameof(weightsQ2K));
        ...
        Span<nint> buffers = stackalloc nint[4] { weightsQ2K.Handle, xq.Handle, xds.Handle, outputC.Handle };
