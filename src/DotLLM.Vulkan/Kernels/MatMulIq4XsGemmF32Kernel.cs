...
/// <summary>
/// IQ4_XS prefill-path batched GEMM: <c>C[N, M] = B[N, K] @ W_iq4xs[M, K]^T</c>.
/// </summary>
/// <remarks>
/// Companion to <see cref="MatMulIq4XsGemvF32Kernel"/>. Same byte layout —
/// each weight row holds <c>K / 256</c> IQ4_XS super-blocks of 136 bytes. The
/// K loop iterates one IQ4_XS *sub-block* (32 elements) per chunk; 8 chunks
/// per super-block. Per chunk we cooperatively dequantise a 16x32 tile into
/// shared memory once and reuse it across all 16 token rows of B.
/// </remarks>
public sealed class MatMulIq4XsGemmF32Kernel : IDisposable
{
    ...
    private MatMulIq4XsGemmF32Kernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool)
    {
        _device = device;
        _module = module;
        _pipeline = pipeline;
    ...
    /// <summary>Loads <c>matmul_iq4_xs_f32_gemm.spv</c> from the given directory and creates the pipeline.</summary>
    public static MatMulIq4XsGemmF32Kernel Create(VulkanDevice device, string spvDir)
    {
        string path = Path.Combine(spvDir, "matmul_iq4_xs_f32_gemm.spv");
        if (!File.Exists(path))
        ...
        return new MatMulIq4XsGemmF32Kernel(device, module, pipeline, pool);
    }
    ...
    public void Launch(
        VulkanDevice.Buffer weightsIq4Xs, VulkanDevice.Buffer inputB, VulkanDevice.Buffer outputC,
        int m, int k, int n)
    {
        using var ctx = _device.CreateSubmitContext();
        ctx.Begin();
        Record(ctx.CommandBuffer, weightsIq4Xs, inputB, outputC, m, k, n);
        ctx.SubmitAndWait();
    }

    /// <summary>Records the IQ4_XS GEMM into <paramref name="cmdBuf"/> without submitting.</summary>
        ...
        int m, int k, int n)
    {
        ...
        Span<nint> buffers = stackalloc nint[3] { weightsIq4Xs.Handle, inputB.Handle, outputC.Handle };
