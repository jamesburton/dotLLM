...
/// <c>y[n, m] = dequant_q5k(bank[indices[n], m, :]) dot x[n, :]</c>.
/// </summary>
/// <remarks>
/// <para>
/// Per-row Q5_K dequantisation in the inner loop — the bank stays in its
/// 176-byte-per-super-block GGUF layout on device. Sibling of
/// <see cref="MoeIndexedMatmulQ4_KF32Kernel"/>; the dequant comes straight
/// from <see cref="MatMulQ5KGemvF32Kernel"/> (and matches
...
/// <see cref="MoeIndexedMatmulQ4_KF32Kernel"/> (identical dispatch shape,
/// only the per-row dequant differs — 5-bit weights via an extra qh[] lookup
/// instead of 4-bit).
/// </para>
/// <para>
/// Added for #372: Qwen3MoeHybrid resident-quant MoE requires this to cover
/// llama.cpp "UD" mixed-quant GGUFs whose down-projection bank quantizes to
/// Q5_K while gate/up stay Q4_K (e.g. the real Qwen3.6-35B-A3B UD-Q4_K_XL
/// checkpoint) — #371 could only wire Q4_K, so those layers fell back to
/// F32 for every bank.
/// </para>
/// </remarks>
public sealed class MoeIndexedMatmulQ5_KF32Kernel : IDisposable
{
    ...
    private MoeIndexedMatmulQ5_KF32Kernel(
    ...
    /// <summary>Loads <c>moe_indexed_matmul_q5_k_f32.spv</c> from <paramref name="spvDir"/>.</summary>
    public static MoeIndexedMatmulQ5_KF32Kernel Create(VulkanDevice device, string spvDir)
    {
        string path = Path.Combine(spvDir, "moe_indexed_matmul_q5_k_f32.spv");
        if (!File.Exists(path))
        ...
        return new MoeIndexedMatmulQ5_KF32Kernel(device, module, pipeline, pool);
    }
    ...
    public void Launch(
        VulkanDevice.Buffer bankQ5K, VulkanDevice.Buffer x, VulkanDevice.Buffer indices, VulkanDevice.Buffer y,
        int m, int k, int n, int numExperts)
    {
        using var ctx = _device.CreateSubmitContext();
        ctx.Begin();
        Record(ctx.CommandBuffer, bankQ5K, x, indices, y, m, k, n, numExperts);
        ctx.SubmitAndWait();
    }

    /// <summary>Records the indexed Q5_K expert-bank matmul dispatch into <paramref name="cmdBuf"/>.</summary>
        ...
        int m, int k, int n, int numExperts)
    {
        ...
        {
            bankQ5K.Handle, x.Handle, indices.Handle, y.Handle,
        };
