...
/// <summary>
/// Q4_K MMQ prefill-path batched GEMM: <c>C[N, M] = B[N, K] @ W_q4k[M, K]^T</c>
/// via integer dp4a. The activation <c>B</c> must already be quantized to Q8_1
/// row-wise (<see cref="QuantizeQ8_1RowsKernel"/>); this kernel decodes the Q4_K
/// nibbles to int8 in shared memory, runs <c>dotPacked4x8AccSatEXT</c> per 32-element
/// sub-block, scales by <c>d·scale·d_x</c>, and subtracts the asymmetric Q4_K min
/// term <c>dmin·min·s_x</c> using the Q8_1 row block-sum.
/// </summary>
/// <remarks>
/// <para>
/// The compute-bound seqLen&gt;1 analogue of <see cref="MatMulQ4KMmvqKernel"/> (decode)
/// and <see cref="MatMulQ8_0MmqKernel"/> (Q8_0 prefill). Replaces the dequant→FP GEMM
/// (<see cref="MatMulQ4KGemmF32Kernel"/>) on the seqLen&gt;1 path on devices that
/// advertise <see cref="VulkanDevice.HasIntegerDotProduct"/> — Q4_K is the most common
/// quant and previously had no int8 prefill path (and no coopmat path), so prefill went
/// through a full F32 weight expansion.
/// </para>
/// <para>
/// Result is NOT bit-exact vs the F32-in GEMM (the activation is int8-quantized first).
/// Validated against the CPU F32 oracle with an argmax-exact + tolerance parity test.
/// </para>
/// <para>
/// Dispatch: 2-D grid, workgroup <c>(16, 16, 1)</c> — one 16×16 output cell of <c>C</c>
/// per workgroup. The 16-row weight tile is nibble-decoded into shared int8 once per
/// 256-element super-block and reused across 16 tokens; the 16-token activation tile is
/// reused across 16 weight rows.
/// </para>
/// </remarks>
public sealed class MatMulQ4KMmqKernel : IDisposable
{
    /// <summary>Q4_K super-block: fp16 d + fp16 dmin + 12 scale bytes + 128 qs bytes.</summary>
    ...
    private MatMulQ4KMmqKernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool)
    {
        _device = device;
        _module = module;
        _pipeline = pipeline;
    ...
    /// <summary>
    /// Loads <c>matmul_q4_k_mmq.spv</c> from <paramref name="spvDir"/> and builds the
    ...
    public static MatMulQ4KMmqKernel? TryCreate(VulkanDevice device, string spvDir)
    {
        ...
        return new MatMulQ4KMmqKernel(device, module, pipeline, pool);
    }
        ...
        int m, int k, int n)
    {
        ...
        ctx.Begin();
        Record(ctx.CommandBuffer, weightsQ4K, xq, xds, outputC, m, k, n);
        ctx.SubmitAndWait();
    }

    /// <summary>Records the MMQ Q4_K GEMM into <paramref name="cmdBuf"/> without submitting.</summary>
        ...
        int m, int k, int n)
    {
        ...
        Span<nint> buffers = stackalloc nint[4] { weightsQ4K.Handle, xq.Handle, xds.Handle, outputC.Handle };
