...
/// <summary>
/// MoE INDEXED Q5_1 MMVQ decode GEMV (issue #137):
/// <c>y[n, m] = downScale[indices[n]] · Σ_k dequant_q5_1(bank[indices[n], m, k]) · x[n, k]</c>
/// via integer dp4a against the row-major Q8_1-quantized expanded MoE
/// activations (<see cref="QuantizeQ8_1RowsKernel"/>). Per 32-element Q5_1
/// block: <c>d·d_x·dot + m·s</c> (s = the Q8_1 block-sum), with the per-expert
/// Gemma-4 <c>ffn_down_exps.scale[e]</c> folded into the final reduced sum —
/// identical fold order to the scalar <see cref="MoeIndexedMatmulQ5_1F32Kernel"/>
/// and the CPU reference.
/// </summary>
/// <remarks>
/// <para>
/// Replaces the one-thread-per-cell scalar
/// <see cref="MoeIndexedMatmulQ5_1F32Kernel"/> on the DECODE path (small
/// expanded-row count) with a COALESCED subgroup-per-cell window sweep
/// (8 × 24-byte blocks = 192 contiguous bytes per iteration).
/// </para>
/// <para>
...
public sealed class MoeIndexedMatmulQ5_1MmvqKernel : IDisposable
{
    ...
    /// <summary>
    /// Loads <c>moe_indexed_matmul_q5_1_mmvq.spv</c> from <paramref name="spvDir"/> and
    ...
    /// <summary>Records the indexed Q5_1 MMVQ expert matmul into <paramref name="cmdBuf"/>.</summary>
    /// <param name="cmdBuf">Open Vulkan command buffer.</param>
    /// <param name="bankQ5_1">Raw Q5_1 bank of <c>numExperts * M * (K/32) * 24</c> bytes.</param>
    ...
    public unsafe void Record(
        nint cmdBuf,
        VulkanDevice.Buffer bankQ5_1, VulkanDevice.Buffer xq, VulkanDevice.Buffer xds,
        VulkanDevice.Buffer indices, VulkanDevice.Buffer y, VulkanDevice.Buffer downScale,
        int m, int k, int n, int numExperts)
    {
        ...
        if (downScale.Size < (long)numExperts * sizeof(float))
            throw new ArgumentException("downScale buffer too small.", nameof(downScale));

        Span<nint> buffers = stackalloc nint[BuffersPerSet]
        {
            bankQ5_1.Handle, xq.Handle, xds.Handle, indices.Handle, y.Handle, downScale.Handle,
        };
