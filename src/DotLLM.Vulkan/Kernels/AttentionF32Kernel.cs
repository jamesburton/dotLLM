...
public sealed class AttentionF32Kernel : IDisposable
{
    ...
    public static AttentionF32Kernel Create(VulkanDevice device, string spvDir)
    {
        ...
        bool subgroupAvailable = device.HasSubgroupArithmetic && !IsForceSharedReduce();
    ...
    /// <summary>
    /// Env-var name: when set to <c>1</c>, forces the shared-mem reference
    /// dispatch on every kernel that has a subgroup / coopmat variant. Used
    /// as the deepest fallback when subgroup arithmetic is reported but
    /// driver bugs make it unsafe (e.g. RDNA3.5 iGPU prerelease drivers).
    /// The wider helper exposing the constant from <c>RmsNormF32Kernel</c>
    /// lives in a follow-up PR; exposed here while that change is outstanding
    /// so tests can toggle the gate without depending on the unmerged commit.
    /// </summary>
    internal const string ForceSharedReduceEnvVar = "DOTLLM_VULKAN_FORCE_SHARED_REDUCE";

    private static bool IsForceSharedReduce() =>
