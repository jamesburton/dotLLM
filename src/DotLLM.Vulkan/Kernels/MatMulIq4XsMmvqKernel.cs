...
/// <summary>
/// IQ4_XS MMVQ decode-path GEMV via integer dp4a against the coalesced
/// lane=K-position layout (issue #339). IQ4_XS reconstructs each weight by a 16-entry
...
/// router falls back to <see cref="MatMulIq4XsGemvF32Kernel"/> otherwise. NOT
/// bit-exact vs the F32-in GEMV — validated argmax + tolerance.
/// </summary>
public sealed class MatMulIq4XsMmvqKernel : IDisposable
{
    /// <summary>IQ4_XS block bytes.</summary>
    public const int Iq4XsBlockBytes = 136;

    /// <summary>Elements per IQ4_XS block.</summary>
    ...
    private MatMulIq4XsMmvqKernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool)
    {
        _device = device;
        _module = module;
        _pipeline = pipeline;
    ...
    /// <summary>Loads <c>matmul_iq4_xs_mmvq.spv</c>; null when the SPV is missing or the device
    /// lacks integer-dot-product support.</summary>
    public static MatMulIq4XsMmvqKernel? TryCreate(VulkanDevice device, string spvDir)
    {
        if (!device.HasIntegerDotProduct)
            return null;

        string path = Path.Combine(spvDir, "matmul_iq4_xs_mmvq.spv");
        if (!File.Exists(path))
            return null;
        ...
        return new MatMulIq4XsMmvqKernel(device, module, pipeline, pool);
    }

    /// <summary>Drops every cached descriptor set; call when scratch buffers have been re-allocated.</summary>
    internal void InvalidateDescriptorCache() => _descriptorCache.Reset();

    /// <summary>Records the IQ4_XS MMVQ GEMV into <paramref name="cmdBuf"/>.</summary>
