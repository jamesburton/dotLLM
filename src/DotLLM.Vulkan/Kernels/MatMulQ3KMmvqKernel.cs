...
/// <summary>
/// Q3_K MMVQ decode-path GEMV: <c>y[M] = W_q3_k[M,K] @ x[K]</c> via integer dp4a
...
/// <see cref="MatMulQ3KGemvF32Kernel"/> otherwise. NOT bit-exact vs the
/// F32-in GEMV (the activation is int8-quantized) — validated argmax + tolerance.
/// </summary>
public sealed class MatMulQ3KMmvqKernel : IDisposable
{
    /// <summary>Q3_K super-block bytes for 256 elements.</summary>
    ...
    private MatMulQ3KMmvqKernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool)
    {
        _device = device;
        _module = module;
        _pipeline = pipeline;
    ...
    public static MatMulQ3KMmvqKernel? TryCreate(VulkanDevice device, string spvDir)
    {
        if (!device.HasIntegerDotProduct)
            return null;

        string path = Path.Combine(spvDir, "matmul_q3_k_mmvq.spv");
        if (!File.Exists(path))
            return null;
        ...
        return new MatMulQ3KMmvqKernel(device, module, pipeline, pool);
    }

    /// <summary>Drops every cached descriptor set; call when scratch buffers have been re-allocated.</summary>
    internal void InvalidateDescriptorCache() => _descriptorCache.Reset();

    /// <summary>Records the Q3_K MMVQ GEMV into <paramref name="cmdBuf"/>.</summary>
