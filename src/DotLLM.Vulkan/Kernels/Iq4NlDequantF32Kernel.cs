...
/// <summary>
/// IQ4_NL → FP32 dequantization. Reads a tightly-packed IQ4_NL blob and
/// produces a contiguous FP32 buffer.
/// </summary>
/// <remarks>
/// Layout matches <c>DotLLM.Cpu.Kernels.Dequantize.DequantizeIQ4_NL</c> and
/// llama.cpp's <c>block_iq4_nl</c>. One workgroup per 32-element block,
/// 32 threads per workgroup, one thread per output element.
/// </remarks>
public sealed class Iq4NlDequantF32Kernel : IDisposable
{
    /// <summary>IQ4_NL block: 2 + 16 = 18 bytes.</summary>
    public const int IQ4_NLBlockBytes = QuantFormat.IQ4_NLBlockBytes;

    /// <summary>Elements per IQ4_NL block.</summary>
    public const int IQ4_NLGroupSize = QuantFormat.LegacyGroupSize;

    private const int PushConstantBytes = 2 * sizeof(uint); // totalBlocks, srcUints
    ...
    private Iq4NlDequantF32Kernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool)
    {
        _device = device;
        _module = module;
        _pipeline = pipeline;
    ...
    /// <summary>Loads <c>iq4_nl_dequant_f32.spv</c> from the given directory and creates the pipeline.</summary>
    public static Iq4NlDequantF32Kernel Create(VulkanDevice device, string spvDir)
    {
        string path = Path.Combine(spvDir, "iq4_nl_dequant_f32.spv");
        if (!File.Exists(path))
        ...
        return new Iq4NlDequantF32Kernel(device, module, pipeline, pool);
    }
        ...
        nint cmdBuf, VulkanDevice.Buffer src, VulkanDevice.Buffer dst, int totalBlocks)
    {
        ...
        long srcMin = (long)totalBlocks * IQ4_NLBlockBytes;
        long dstMin = (long)totalBlocks * IQ4_NLGroupSize * sizeof(float);
        if (src.Size < srcMin)
