...
/// <summary>
/// IQ2_XS MMVQ decode-path GEMV via integer dp4a (issue #339). The activation must
/// already be Q8_1 (<see cref="QuantizeQ8_1Kernel"/>); each weight is reconstructed
/// from the shared 512-entry <c>iq2xs_grid</c> + 128-entry <c>ksigns</c> codebooks
/// (<see cref="Iq2Codebooks"/>) as sign·grid, which is int8 → fed to dp4a. 6 bindings:
...
/// <see cref="MatMulIq2XsGemvF32Kernel"/> otherwise. NOT bit-exact — argmax + tolerance.
/// </summary>
public sealed class MatMulIq2XsMmvqKernel : IDisposable
{
    /// <summary>IQ2_XS super-block bytes.</summary>
    public const int Iq2XsBlockBytes = QuantFormat.IQ2_XSBlockBytes;
    /// <summary>Elements per IQ2_XS super-block.</summary>
    public const int Iq2XsGroupSize = QuantFormat.KQuantGroupSize;
    ...
    private MatMulIq2XsMmvqKernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool, Iq2Codebooks codebooks)
    {
        _device = device;
        _module = module;
        _pipeline = pipeline;
    ...
    /// <summary>Loads <c>matmul_iq2_xs_mmvq.spv</c> and reuses the shared <paramref name="codebooks"/>;
    /// null when the SPV is missing or the device lacks integer-dot-product support.</summary>
    internal static MatMulIq2XsMmvqKernel? TryCreate(VulkanDevice device, string spvDir, Iq2Codebooks codebooks)
    {
        if (!device.HasIntegerDotProduct)
            return null;

        string path = Path.Combine(spvDir, "matmul_iq2_xs_mmvq.spv");
        if (!File.Exists(path))
            return null;
        ...
        return new MatMulIq2XsMmvqKernel(device, module, pipeline, pool, codebooks);
    }

    /// <summary>Drops every cached descriptor set; call when scratch buffers have been re-allocated.</summary>
    internal void InvalidateDescriptorCache() => _descriptorCache.Reset();

    /// <summary>Records the IQ2_XS MMVQ GEMV into <paramref name="cmdBuf"/>.</summary>
        ...
        int m, int k)
    {
        ...
        if ((k % Iq2XsGroupSize) != 0)
            throw new ArgumentException($"k must be a multiple of {Iq2XsGroupSize}, got {k}", nameof(k));

        int blocksPerRow = k / Iq2XsGroupSize;
        long rowBytes = (long)blocksPerRow * Iq2XsBlockBytes;
