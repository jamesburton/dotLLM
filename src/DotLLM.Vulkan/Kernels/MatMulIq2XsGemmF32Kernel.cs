...
/// <summary>
/// IQ2_XS prefill-path batched GEMM: <c>C[N, M] = B[N, K] @ W_iq2xs[M, K]^T</c>.
/// </summary>
public sealed class MatMulIq2XsGemmF32Kernel : IDisposable
{
    ...
    private MatMulIq2XsGemmF32Kernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool,
    ...
    public static MatMulIq2XsGemmF32Kernel Create(VulkanDevice device, string spvDir)
    {
    ...
    internal static MatMulIq2XsGemmF32Kernel CreateWithCodebooks(VulkanDevice device, string spvDir, Iq2Codebooks codebooks)
        => CreateInternal(device, spvDir, codebooks, ownsCodebooks: false);

    private static MatMulIq2XsGemmF32Kernel CreateInternal(VulkanDevice device, string spvDir, Iq2Codebooks codebooks, bool ownsCodebooks)
    {
        string path = Path.Combine(spvDir, "matmul_iq2_xs_f32_gemm.spv");
        if (!File.Exists(path))
        ...
        return new MatMulIq2XsGemmF32Kernel(device, module, pipeline, pool, codebooks, ownsCodebooks);
    }
        ...
        int m, int k, int n)
    {
        ...
        {
            weights.Handle, inputB.Handle, outputC.Handle,
