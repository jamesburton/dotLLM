...
/// <summary>
/// PQ2_0 token-embedding gather + dequantize (issue #352): gathers one
/// embedding row per input token from a device-resident RAW PQ2_0 table,
...
/// the table in its PQ2_0 byte layout (34 bytes per 32 weights vs 128) saves
/// ~3.76x its device bytes — ~772 MB on Llama-3.2-1B-Instruct-PQ2_0.
/// </para>
/// <para>
/// Layout matches <c>DotLLM.Cpu.Kernels.Dequantize.DequantizePQ2_0Scalar</c> and
...
/// </para>
/// </remarks>
public sealed class Pq2_0EmbedGatherF32Kernel : IDisposable
{
    /// <summary>PQ2_0 group: 2-byte fp16 scale + 32 packed code bytes = 34 bytes per 128 weights.</summary>
    public const int Pq2_0BlockBytes = 34;

    /// <summary>Weights per PQ2_0 group.</summary>
    ...
    private Pq2_0EmbedGatherF32Kernel(
    ...
    /// <summary>Loads <c>pq2_0_embed_gather_f32.spv</c> from <paramref name="spvDir"/>.</summary>
    public static Pq2_0EmbedGatherF32Kernel Create(VulkanDevice device, string spvDir)
    {
        string path = Path.Combine(spvDir, "pq2_0_embed_gather_f32.spv");
        if (!File.Exists(path))
        ...
        return new Pq2_0EmbedGatherF32Kernel(device, module, pipeline, pool);
    }
    ...
    /// </summary>
    public static Pq2_0EmbedGatherF32Kernel? TryCreate(VulkanDevice device, string? spvDir)
    {
        if (spvDir is null) return null;
        if (!File.Exists(Path.Combine(spvDir, "pq2_0_embed_gather_f32.spv"))) return null;
        ...
        int nTokens, int hidden, int vocabSize)
    {
        ...
        if (hidden % Pq2_0GroupSize != 0)
            throw new ArgumentException(
                $"PQ2_0 embedding rows need hidden % {Pq2_0GroupSize} == 0, got {hidden}.", nameof(hidden));

        int blocksPerRow = hidden / Pq2_0GroupSize;
        long tableMin = (long)vocabSize * blocksPerRow * Pq2_0BlockBytes;
