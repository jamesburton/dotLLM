...
/// Mirrors <c>DotLLM.Cpu.Kernels.FusedOps.GeGLUTanh</c> — the Gemma 2 / Gemma 3
/// MLP activation. Shape-identical to <see cref="SwiGluF32Kernel"/>; only the
/// gate activation differs (GELU-tanh vs SiLU).
/// </summary>
/// <remarks>
/// Pointwise, no reduction — one thread per output element.
/// </remarks>
public sealed class GeGluTanhF32Kernel : IDisposable
{
    ...
    private GeGluTanhF32Kernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool)
    {
        _device = device;
        _module = module;
        _pipeline = pipeline;
    ...
    /// <summary>Loads <c>geglu_tanh_f32.spv</c> from the given directory and creates the pipeline.</summary>
    public static GeGluTanhF32Kernel Create(VulkanDevice device, string spvDir)
    {
        string path = Path.Combine(spvDir, "geglu_tanh_f32.spv");
        if (!File.Exists(path))
        ...
        return new GeGluTanhF32Kernel(device, module, pipeline, pool);
    }
    ...
    /// <summary>
    /// Dispatches GeGLU over <paramref name="n"/> elements. Synchronous —
    /// returns after <c>vkQueueWaitIdle</c>.
    /// </summary>
    /// <param name="gate">FP32 gate buffer (pre-activation).</param>
    /// <param name="up">FP32 up buffer.</param>
    /// <param name="result">FP32 output buffer (distinct from <paramref name="up"/>).</param>
    ...
    /// <summary>Records GeGLU into <paramref name="cmdBuf"/> without submitting.</summary>
