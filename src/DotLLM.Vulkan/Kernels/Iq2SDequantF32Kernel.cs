...
/// <summary>
/// IQ2_S → FP32 dequantization. Reads a tightly-packed IQ2_S blob and produces
/// a contiguous FP32 buffer. Also handles MOSTLY_IQ2_M file-type tensors —
/// ggml uses the same block layout for both presets.
/// </summary>
/// <remarks>
/// Layout matches <c>DotLLM.Cpu.Kernels.Dequantize.DequantizeIQ2_S</c> and
/// llama.cpp's <c>block_iq2_s</c>: 82 bytes per 256-element super-block,
/// 1024-entry codebook with high index bits in <c>qh</c>; 8-bit sign mask
/// stored explicitly per pair (no ksigns indirection).
/// </remarks>
public sealed class Iq2SDequantF32Kernel : IDisposable
{
    /// <summary>IQ2_S super-block: 2 + 32 + 32 + 8 + 8 = 82 bytes.</summary>
    public const int IQ2_SBlockBytes = QuantFormat.IQ2_SBlockBytes;

    /// <summary>Elements per IQ2_S super-block.</summary>
    public const int IQ2_SGroupSize = QuantFormat.KQuantGroupSize;
    ...
    private Iq2SDequantF32Kernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool,
    ...
    public static Iq2SDequantF32Kernel Create(VulkanDevice device, string spvDir)
    {
    ...
    internal static Iq2SDequantF32Kernel CreateWithCodebooks(VulkanDevice device, string spvDir, Iq2Codebooks codebooks)
        => CreateInternal(device, spvDir, codebooks, ownsCodebooks: false);

    private static Iq2SDequantF32Kernel CreateInternal(VulkanDevice device, string spvDir, Iq2Codebooks codebooks, bool ownsCodebooks)
    {
        string path = Path.Combine(spvDir, "iq2_s_dequant_f32.spv");
        if (!File.Exists(path))
        ...
        return new Iq2SDequantF32Kernel(device, module, pipeline, pool, codebooks, ownsCodebooks);
    }
        ...
        nint cmdBuf, VulkanDevice.Buffer src, VulkanDevice.Buffer dst, int totalSuperblocks)
    {
        ...
        long srcMin = (long)totalSuperblocks * IQ2_SBlockBytes;
        long dstMin = (long)totalSuperblocks * IQ2_SGroupSize * sizeof(float);
        if (src.Size < srcMin)
