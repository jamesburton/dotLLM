...
/// <summary>
/// IQ2_XXS MMVQ decode-path GEMV via integer dp4a (issue #339). The activation must
/// already be Q8_1 (<see cref="QuantizeQ8_1Kernel"/>); each weight is reconstructed
/// from the shared 256-entry <c>iq2xxs_grid</c> + 128-entry <c>ksigns</c> codebooks
...
/// <see cref="MatMulIq2XxsGemvF32Kernel"/> otherwise. NOT bit-exact — argmax + tolerance.
/// </summary>
public sealed class MatMulIq2XxsMmvqKernel : IDisposable
{
    ...
    public const int Iq2XxsGroupSize = 256;
    ...
    private MatMulIq2XxsMmvqKernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool, Iq2Codebooks codebooks)
    {
        _device = device;
        _module = module;
        _pipeline = pipeline;
    ...
    /// <summary>Loads <c>matmul_iq2_xxs_mmvq.spv</c> and reuses the shared <paramref name="codebooks"/>;
    /// null when the SPV is missing or the device lacks integer-dot-product support.</summary>
    internal static MatMulIq2XxsMmvqKernel? TryCreate(VulkanDevice device, string spvDir, Iq2Codebooks codebooks)
    {
        if (!device.HasIntegerDotProduct)
            return null;

        string path = Path.Combine(spvDir, "matmul_iq2_xxs_mmvq.spv");
        if (!File.Exists(path))
            return null;
        ...
        return new MatMulIq2XxsMmvqKernel(device, module, pipeline, pool, codebooks);
    }

    /// <summary>Drops every cached descriptor set; call when scratch buffers have been re-allocated.</summary>
    internal void InvalidateDescriptorCache() => _descriptorCache.Reset();

    /// <summary>Records the IQ2_XXS MMVQ GEMV into <paramref name="cmdBuf"/>.</summary>
