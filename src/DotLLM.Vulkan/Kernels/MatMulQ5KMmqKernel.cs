...
/// <summary>
/// Q5_K MMQ prefill-path batched GEMM: <c>C[N, M] = B[N, K] @ W_q5k[M, K]^T</c>
/// via integer dp4a. Sibling of <see cref="MatMulQ4KMmqKernel"/> — identical tiling
/// and per-sub-block scale + asymmetric min term, plus one qh 5th-bit lookup per
/// element (q5 = nibble | qh_bit&lt;&lt;4, 0..31). The activation <c>B</c> must already
/// be Q8_1 row-wise (<see cref="QuantizeQ8_1RowsKernel"/>).
/// </summary>
/// <remarks>
...
{
    /// <summary>Q5_K super-block: d + dmin + scales[12] + qh[32] + qs[128].</summary>
    ...
    /// <summary>
    /// Loads <c>matmul_q5_k_mmq.spv</c> from <paramref name="spvDir"/> and builds the
    ...
    /// <see cref="MatMulQ5KGemmF32Kernel"/>.
    /// </summary>
    public static MatMulQ5KMmqKernel? TryCreate(VulkanDevice device, string spvDir)
    {
        if (!device.HasIntegerDotProduct)
            return null;

        string path = Path.Combine(spvDir, "matmul_q5_k_mmq.spv");
        if (!File.Exists(path))
            return null;
        ...
        return new MatMulQ5KMmqKernel(device, module, pipeline, pool);
    }
        ...
        int m, int k, int n)
    {
        ...
        ctx.Begin();
        Record(ctx.CommandBuffer, weightsQ5K, xq, xds, outputC, m, k, n);
        ctx.SubmitAndWait();
    }

    /// <summary>Records the MMQ Q5_K GEMM into <paramref name="cmdBuf"/> without submitting.</summary>
    /// <param name="cmdBuf">Open Vulkan command buffer to append commands to.</param>
    /// <param name="weightsQ5K">Raw Q5_K blob of <c>M * (K / 256) * 176</c> bytes, rows contiguous.</param>
        ...
        int m, int k, int n)
    {
        ...
        Span<nint> buffers = stackalloc nint[4] { weightsQ5K.Handle, xq.Handle, xds.Handle, outputC.Handle };
