...
/// <summary>
/// Q6_K prefill-path batched GEMM: <c>C[N, M] = B[N, K] @ W_q6k[M, K]^T</c>.
/// </summary>
/// <remarks>
/// <para>
/// Companion to <see cref="MatMulQ6KGemvF32Kernel"/> (decode-path GEMV) and
/// structural sibling of <see cref="MatMulQ4KGemmF32Kernel"/> /
/// <see cref="MatMulQ5KGemmF32Kernel"/>: same tiling (16x16 output tile per
/// workgroup, K-chunk = 32 elements, 8 K-chunks per super-block). Q6_K's
/// dequant is structurally simpler than Q4_K / Q5_K — scale-only
/// reconstruction with 16 signed <c>int8</c> scales and signed quants in
/// <c>-32..31</c>. See <see cref="MatMulQ6KGemvF32Kernel"/> for the full
/// byte-extraction commentary.
/// </para>
/// <para>
/// Tiling: 16x16 output tile per workgroup (one thread per output cell). The
/// K-axis is iterated in chunks of 32 elements (one Q6_K (half, group)
/// pair); 8 such chunks per super-block. Per K-chunk we cooperatively
/// dequantise a 16x32 weight tile into shared memory once and reuse it
/// across all 16 token rows of B. No subgroup / cooperative-matrix
/// intrinsics in this Phase 1 kernel — broadest driver portability and
/// correctness first; perf follow-ups (coopmat, F16 acc) tracked separately.
/// </para>
/// </remarks>
public sealed class MatMulQ6KGemmF32Kernel : IDisposable
{
    ...
    private MatMulQ6KGemmF32Kernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool)
    {
        _device = device;
        _module = module;
        _pipeline = pipeline;
    ...
    /// <summary>Loads <c>matmul_q6_k_gemm_f32.spv</c> from the given directory and creates the pipeline.</summary>
    public static MatMulQ6KGemmF32Kernel Create(VulkanDevice device, string spvDir)
    {
        string path = Path.Combine(spvDir, "matmul_q6_k_gemm_f32.spv");
        if (!File.Exists(path))
        ...
        return new MatMulQ6KGemmF32Kernel(device, module, pipeline, pool);
    }
    ...
    /// <summary>
    /// Dispatches the batched GEMM: <c>C[N, M] = B[N, K] @ W_q6k[M, K]^T</c>.
    ...
    /// <summary>Records the Q6_K GEMM into <paramref name="cmdBuf"/> without submitting.</summary>
