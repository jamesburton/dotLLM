...
/// <summary>
/// In-place squared-ReLU activation: <c>x[i] = max(0, x[i])^2</c>.
/// Mirrors <c>DotLLM.Cpu.Kernels.ReluSquared.Execute</c>.
/// </summary>
/// <remarks>
/// NVIDIA Nemotron-H MLP layers use squared-ReLU as a non-gated replacement
/// for SwiGLU — the existing fused <see cref="SwiGluF32Kernel"/> can't be
/// reused there because it computes <c>silu(gate) * up</c> across two distinct
/// buffers, while squared-ReLU rewrites the post-up-projection activation
/// buffer in place. Pointwise, no reduction, one thread per element.
/// </remarks>
public sealed class ReluSquaredInplaceF32Kernel : IDisposable
{
    ...
    private ReluSquaredInplaceF32Kernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool)
    {
        _device = device;
        _module = module;
        _pipeline = pipeline;
    ...
    public static ReluSquaredInplaceF32Kernel Create(VulkanDevice device, string spvDir)
    {
        string path = Path.Combine(spvDir, "relu_squared_inplace_f32.spv");
        if (!File.Exists(path))
        ...
        return new ReluSquaredInplaceF32Kernel(device, module, pipeline, pool);
    }
    ...
    /// <summary>
    /// Records the in-place squared-ReLU dispatch. <paramref name="x"/> is treated
