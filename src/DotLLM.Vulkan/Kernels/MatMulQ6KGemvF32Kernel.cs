...
/// <remarks>
/// <para>
/// Same dispatch model as the Q8_0 GEMV (one workgroup per output row, 128
/// threads, shared-memory tree reduce); the Q4_K / Q5_K GEMV siblings (landing
/// in parallel extraction PRs) share the same shape. Q6_K is structurally
/// simpler than Q4_K / Q5_K on the metadata side: scale-only reconstruction
/// (no <c>dmin</c>), 16 signed <c>int8</c> scales (no 6-bit packed scale
/// table), and signed quants in <c>-32..31</c>. The byte extraction, however,
/// is more involved because a Q6_K super-block is laid out as two 128-element
/// halves, each with 4 groups of 32 elements that share a 32-byte <c>qh</c>
/// slab.
/// </para>
/// <para>
...
/// decode-path, matching <see cref="MatMulQ8_0Kernel"/>. Output <c>y</c> is
/// FP32.
/// </para>
/// <para>
