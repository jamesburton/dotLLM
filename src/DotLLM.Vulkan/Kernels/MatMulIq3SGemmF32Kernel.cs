...
/// <summary>
/// IQ3_S prefill-path batched GEMM: <c>C[N, M] = B[N, K] @ W_iq3s[M, K]^T</c>.
/// </summary>
public sealed class MatMulIq3SGemmF32Kernel : IDisposable
{
    /// <summary>IQ3_S super-block size in bytes.</summary>
    public const int IQ3_SBlockBytes = QuantFormat.IQ3_SBlockBytes;
    /// <summary>Elements per IQ3_S super-block.</summary>
    public const int IQ3_SGroupSize = QuantFormat.KQuantGroupSize;
    ...
    private MatMulIq3SGemmF32Kernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool,
    ...
    /// <summary>Loads <c>matmul_iq3_s_f32_gemm.spv</c>, allocates the shared codebook
    /// SSBOs, and creates the pipeline.</summary>
    public static MatMulIq3SGemmF32Kernel Create(VulkanDevice device, string spvDir)
    {
    ...
    internal static MatMulIq3SGemmF32Kernel CreateWithCodebooks(VulkanDevice device, string spvDir, Iq3Codebooks codebooks)
        => CreateInternal(device, spvDir, codebooks, ownsCodebooks: false);

    private static MatMulIq3SGemmF32Kernel CreateInternal(VulkanDevice device, string spvDir, Iq3Codebooks codebooks, bool ownsCodebooks)
    {
        string path = Path.Combine(spvDir, "matmul_iq3_s_f32_gemm.spv");
        if (!File.Exists(path))
        ...
        return new MatMulIq3SGemmF32Kernel(device, module, pipeline, pool, codebooks, ownsCodebooks);
    }
        ...
        int m, int k, int n)
    {
        ...
        {
            weights.Handle, inputB.Handle, outputC.Handle, _codebooks.Iq3SGrid.Handle,
        };
