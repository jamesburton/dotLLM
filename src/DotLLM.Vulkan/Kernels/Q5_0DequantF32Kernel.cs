...
/// </remarks>
public sealed class Q5_0DequantF32Kernel : IDisposable
{
    ...
    private Q5_0DequantF32Kernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool)
    {
        _device = device;
        _module = module;
        _pipeline = pipeline;
    ...
    public static Q5_0DequantF32Kernel Create(VulkanDevice device, string spvDir)
    {
        string path = Path.Combine(spvDir, "q5_0_dequant_f32.spv");
        if (!File.Exists(path))
        ...
        return new Q5_0DequantF32Kernel(device, module, pipeline, pool);
    }
