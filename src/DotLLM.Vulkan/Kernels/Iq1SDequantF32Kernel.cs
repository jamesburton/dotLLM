...
/// <summary>
/// IQ1_S → FP32 dequantization. Reads a tightly-packed IQ1_S blob (50 bytes
/// per 256-element super-block) and produces a contiguous FP32 buffer.
/// </summary>
/// <remarks>
/// Layout matches <c>DotLLM.Cpu.Kernels.Dequantize.DequantizeIQ1_S</c> and
/// llama.cpp's <c>block_iq1_s</c>. One workgroup per 256-element super-block,
/// 256 threads per workgroup, one thread per output element. The 2048-entry
/// signed-int8 codebook is embedded as a 16 KB <c>const uint[4096]</c> in
/// the shader (each ggml <c>uint64</c> entry split into a low/high uint pair).
/// </remarks>
public sealed class Iq1SDequantF32Kernel : IDisposable
{
    /// <summary>IQ1_S super-block: 2 + 32 + 16 = 50 bytes.</summary>
    public const int IQ1_SBlockBytes = QuantFormat.IQ1_SBlockBytes;

    /// <summary>Elements per IQ1_S super-block.</summary>
    public const int IQ1_SGroupSize = QuantFormat.KQuantGroupSize;
    ...
    private Iq1SDequantF32Kernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool)
    {
        _device = device;
        _module = module;
        _pipeline = pipeline;
    ...
    /// <summary>Loads <c>iq1_s_dequant_f32.spv</c> from the given directory and creates the pipeline.</summary>
    public static Iq1SDequantF32Kernel Create(VulkanDevice device, string spvDir)
    {
        string path = Path.Combine(spvDir, "iq1_s_dequant_f32.spv");
        if (!File.Exists(path))
        ...
        return new Iq1SDequantF32Kernel(device, module, pipeline, pool);
    }
        ...
        nint cmdBuf, VulkanDevice.Buffer src, VulkanDevice.Buffer dst, int totalSuperblocks)
    {
        ...
        long srcMin = (long)totalSuperblocks * IQ1_SBlockBytes;
        long dstMin = (long)totalSuperblocks * IQ1_SGroupSize * sizeof(float);
        if (src.Size < srcMin)
