...
/// element per thread.
/// </remarks>
public sealed class Q2KDequantF32Kernel : IDisposable
{
    ...
    private Q2KDequantF32Kernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool)
    {
        _device = device;
        _module = module;
        _pipeline = pipeline;
    ...
    public static Q2KDequantF32Kernel Create(VulkanDevice device, string spvDir)
    {
        string path = Path.Combine(spvDir, "q2_k_dequant_f32.spv");
        if (!File.Exists(path))
        ...
        return new Q2KDequantF32Kernel(device, module, pipeline, pool);
    }
