...
/// <summary>
/// Q6_K → FP32 dequantization. Reads a tightly-packed Q6_K blob and produces a
/// contiguous FP32 buffer.
/// </summary>
/// <remarks>
/// Layout matches <c>DotLLM.Cpu.Kernels.DequantizeKQuants.DequantizeQ6_K</c> and
/// llama.cpp's <c>block_q6_K</c>: 210 bytes per 256 elements (128 bytes of low
/// 4-bit quants, 64 bytes of high 2-bit quants, 16 int8 scales, fp16 <c>d</c>).
...
/// </remarks>
public sealed class Q6KDequantF32Kernel : IDisposable
{
    ...
    private Q6KDequantF32Kernel(VulkanDevice device, VulkanModule module, ComputePipeline pipeline, nint pool)
    {
        _device = device;
        _module = module;
        _pipeline = pipeline;
    ...
    /// <summary>Loads <c>q6_k_dequant_f32.spv</c> from the given directory and creates the pipeline.</summary>
    public static Q6KDequantF32Kernel Create(VulkanDevice device, string spvDir)
    {
        string path = Path.Combine(spvDir, "q6_k_dequant_f32.spv");
        if (!File.Exists(path))
        ...
        return new Q6KDequantF32Kernel(device, module, pipeline, pool);
    }
        ...
        nint cmdBuf, VulkanDevice.Buffer src, VulkanDevice.Buffer dst, long totalBlocks)
    {
        ...
        long srcMin = totalBlocks * Q6_KBlockBytes;
        long dstMin = totalBlocks * Q6_KGroupSize * sizeof(float);
        if (src.Size < srcMin)
