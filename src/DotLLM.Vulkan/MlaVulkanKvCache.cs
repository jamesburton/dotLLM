...
public sealed class MlaVulkanKvCache : IKvCache, IDisposable
{
        ...
        int numHeads, int qkNopeHeadDim, int vHeadDim, int qkRopeHeadDim)
    {
        ...
        {
            // NOTE: forward-reference adaptation. The source feature branch
            // used device.AllocateDeviceLocal here, but the upstream Vulkan
            // scaffold (PR #205) only exposes host-visible Allocate. Vulkan
            // perf chain converts these to device-local with a staging path.
            _kNopeBuffers[i] = device.Allocate(kNopeBytesPerLayer);
            _vBuffers[i] = device.Allocate(vBytesPerLayer);
            _kPeBuffers[i] = device.Allocate(kPeBytesPerLayer);
        }
    }
