...
public sealed class VulkanMamba3TransformerModel : IModel
{
    ...
    // IQ2 family — IQ-family follow-up. Shared codebooks (3 grids + ksigns).
    ...
    // (KHR_coopmat exposes F16 / Sint8 operands, not BF16).
        ...
        ModelConfig config, Mamba3Weights cpuWeights, string spvDir)
    {
        ...
        // IQ2 family — IQ-family follow-up. Always created. Codebooks shared.
        ...
        // F16 / BF16 native matmul kernels — Phase 8. Always created. F16 GEMM coopmat is
        // opportunistic (null on devices without VK_KHR_cooperative_matrix).
        ...
        int outputDim, int inputDim, int seqLen)
    {
        ...
        else if (weightQt == QuantizationType.F16)
        {
            ...
            // coopmat or scalar tiled GEMM at prefill.
            if (seqLen == 1)
            {
        ...
        else if (weightQt == QuantizationType.BF16)
        {
            // Phase 8: native BF16 weights — 2 bytes/element on device. No coopmat
            // path for BF16 on this hardware.
            if (seqLen == 1)
            {
